...
const lessons: Record<string, Lesson> = {
  ...
  "building-ai-agent": {
    intro: "A simple useful agent needs a goal, a model, a small set of well-described tools, a state store, and clear stop conditions.",
    analogy: "Give a new employee a job description, approved software, a checklist, a spending limit, and a manager for exceptions. Those boundaries make action useful and safe.",
    steps: ["Choose one narrow task and success metric.", "Implement read-only tools first.", "Run a bounded model–tool loop.", "Add validation, approvals, logs, evaluations, and safe failure behavior."],
    exampleTitle: "A documentation assistant",
    example: "The agent searches approved documentation and drafts an answer with citations. If no source supports the answer, it stops and asks a human instead of guessing.",
    code: `MAX_STEPS = 5\nstate = {"goal": user_question, "evidence": []}\nfor _ in range(MAX_STEPS):\n    decision = model.decide(state, tools=[search_docs])\n    if decision.type == "final":\n        return require_citations(decision.answer)\n    state["evidence"].append(search_docs(**decision.args))\nreturn "I couldn't verify this. Please ask a human."`,
    mistake: "Beginning with write, purchase, delete, or send-message tools creates unnecessary risk. Prove quality with read-only tools first.",
    takeaway: "Build the smallest bounded agent that solves one job, then earn more capability through evaluation."
  },
...
function getLearningPath(topicId: string): LearningPath {
  const agentTopics = ["agentic-ai-intro", "tool-calling", "agent-context-engineering", "agent-memory", "planning-reflection", "agent-state-graphs", "durable-long-running-agents", "agentic-rag", "building-ai-agent", "multi-agent-systems", "model-context-protocol", "agent-frameworks", "browser-computer-use-agents", "agent-security", "agent-evaluation-safety", "agent-observability-deployment"];
