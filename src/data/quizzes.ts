...
const quizData: Record<string, QuizQuestion[]> = {
  ...
  "what-is-ml": [
    { id:1, question:"A spam filter learns to improve as it processes more emails. What makes it a machine learning system rather than a traditional program?", options:["It runs on a server","It improves from data rather than following hand-coded rules","It processes emails faster","It is written in Python"], correctAnswerIndex:1, explanation:"The defining characteristic of ML is learning from data. A traditional program has fixed rules written by a programmer. The spam filter updates its understanding of spam patterns automatically as it sees more examples." },
    { id:2, question:"Which problem is machine learning clearly NOT the right tool for?", options:["Predicting which customers will churn next month","Detecting fraudulent transactions in real time","Calculating the area of a rectangle given its width and height","Classifying medical images as cancerous or benign"], correctAnswerIndex:2, explanation:"Area = width × height is a known formula. Writing one line of code is faster, more reliable, and more accurate than training a model. ML is valuable when rules are too complex to specify or when patterns must be discovered from data." },
    { id:3, question:"A model trained to classify cats vs dogs is tested on horse vs zebra images without retraining. What will most likely happen?", options:["It works perfectly — animals share enough features","Performance will be poor — the model learned cat/dog-specific patterns that don't transfer to horses and zebras","It will automatically adapt to the new classes","It will refuse to make predictions"], correctAnswerIndex:1, explanation:"ML models only generalise within the distribution they were trained on. The features the model learned (cat ears, dog fur patterns) are not the right features for distinguishing horses from zebras." },
    { id:4, question:"What is the difference between a model's training error and its generalisation error?", options:["Training error is always higher than generalisation error","Training error measures performance on data the model has seen; generalisation error measures performance on unseen data — generalisation error is what actually matters in production","They are identical for well-trained models","Generalisation error only applies to neural networks"], correctAnswerIndex:1, explanation:"A model that memorises training examples can achieve near-zero training error while failing completely on new data. Generalisation error — performance on unseen data — is the only metric that predicts real-world usefulness." },
  ],
  ...
  "types-of-ml": [
    ...
    { id:3, question:"Which scenario is most suitable for supervised learning?", options:["Finding unusual patterns in network traffic with no known attack signatures","Teaching a drone to fly by letting it crash and recover thousands of times","Predicting tomorrow's sales using 3 years of historical daily sales records","Grouping news articles by topic without category labels"], correctAnswerIndex:2, explanation:"Historical sales with known values provide labelled (date features, sales amount) pairs — exactly what supervised learning needs. The others lack labels or require environment interaction." },
    { id:4, question:"You have 500 labelled examples and 50,000 unlabelled examples of the same task. Which learning paradigm is designed for this?", options:["Supervised learning — just use the 500 labelled examples","Unsupervised learning — ignore the labels entirely","Semi-supervised learning — uses both labelled and unlabelled data together","Reinforcement learning — convert labels into rewards"], correctAnswerIndex:2, explanation:"Semi-supervised learning is designed for exactly this scenario: a small amount of labelled data plus a large pool of unlabelled data. Methods like self-training and label propagation leverage the structure in unlabelled data." },
  ],

  "supervised-learning-intro": [
    { id:1, question:"In supervised learning, what is a 'label'?", options:["The name assigned to a feature column","The known correct output for a training example — what the model is trying to learn to predict","The column header in a CSV file","The model's predicted output before training is complete"], correctAnswerIndex:1, explanation:"A label is the ground truth for each training example. For a loan default predictor, the label is whether that historical borrower actually defaulted. The model learns the mapping from input features to these known labels." },
    { id:2, question:"Your training accuracy is 98% but test accuracy is 61%. What does this most likely indicate?", options:["The test set has data quality problems","Underfitting — the model is too simple to learn","Overfitting — the model memorised training examples instead of learning generalisable patterns","Normal performance — a 37-point gap is expected"], correctAnswerIndex:2, explanation:"A large gap between training and test accuracy is the defining symptom of overfitting. The model learned the specific quirks of the 98% training examples rather than patterns that hold for new data." },
    { id:3, question:"What is the primary risk of having very few training examples?", options:["The model trains too slowly","The model cannot be exported or deployed","Overfitting — with few examples the model can memorise them without learning general patterns","Labels become incorrect automatically when data is small"], correctAnswerIndex:2, explanation:"With very few examples, a model can memorise each one perfectly (near-zero training error) without learning any patterns that generalise. More data is the most reliable cure for overfitting." },
  ...
  "unsupervised-learning-intro": [
    { id:1, question:"Which task genuinely requires no labels?", options:["Predicting whether a customer will cancel their subscription","Grouping 10 million songs into musical styles based on audio features","Classifying emails as spam or not spam","Forecasting quarterly revenue from historical data"], correctAnswerIndex:1, explanation:"Grouping songs by musical style requires no predefined categories and no human to say which group each song belongs to — this is exactly unsupervised learning." },
    { id:2, question:"K-Means was given no category labels, yet it produces 3 clusters. Why?", options:["It reads category names from the column headers","k=3 was specified as a hyperparameter before training began","It automatically detects the right number by trying all values","It copies labels from the last column of the dataset"], correctAnswerIndex:1, explanation:"K-Means requires you to specify k before running. The algorithm finds the best 3 clusters it can, but it cannot discover whether 3 is the right number — that judgment stays with the user." },
    ...
    { id:4, question:"Why can't you use accuracy to evaluate a clustering algorithm?", options:["Clustering only works on images where accuracy doesn't apply","There are no ground-truth labels to compare predictions against — the whole point is that labels don't exist","Clusters always achieve 100% accuracy by construction","Accuracy requires exactly two classes"], correctAnswerIndex:1, explanation:"Clustering is unsupervised — there are no labels to compare against. Use Silhouette Score (how tight and separated clusters are) or Inertia (within-cluster sum of squares) instead." },
  ],

  "reinforcement-learning-intro": [
    { id:1, question:"What are the three core components of any reinforcement learning problem?", options:["Dataset, model, and loss function","Agent, environment, and reward signal","Training set, validation set, and test set","Features, labels, and predictions"], correctAnswerIndex:1, explanation:"RL is defined by an agent that takes actions in an environment and receives reward signals. Unlike supervised learning, there is no pre-existing dataset — the agent generates experience through interaction." },
    ...
    { id:4, question:"What is the exploration-exploitation dilemma?", options:["Choosing between supervised and unsupervised learning","Balancing between trying new actions to discover better strategies (exploration) versus repeating actions already known to give good rewards (exploitation)","Deciding whether to use a GPU or CPU for training","Choosing the number of hidden layers in a neural network"], correctAnswerIndex:1, explanation:"Too much exploitation: the agent settles for a suboptimal strategy and never discovers something better. Too much exploration: the agent never capitalises on what it has already learned. Balancing these is central to RL algorithm design." },
  ],
  ...
  "batch-vs-online": [
    ...
    { id:2, question:"What is the primary disadvantage of batch learning for large and growing datasets?", options:["Batch learning cannot handle numerical features","Each retraining requires processing the entire dataset from scratch — this becomes increasingly expensive and slow as the dataset grows over time","Batch learning models cannot generalise to new data","Batch learning requires an internet connection during training"], correctAnswerIndex:1, explanation:"Batch learning retrains from scratch each time. For a dataset that grows by 1 million examples per day, retraining daily on all accumulated data quickly becomes impractical." },
    { id:3, question:"In online learning, what is the role of the learning rate?", options:["It controls how many examples are processed per second","It controls how much each new example influences the model — high rate means fast adaptation but risk of forgetting old patterns; low rate preserves past knowledge but adapts slowly to genuine change","It sets the initial model weights before any examples arrive","It controls how many features are used per update"], correctAnswerIndex:1, explanation:"This tradeoff is the core tuning challenge in online learning. A learning rate that is too high adapts to noise; one that is too low misses real distribution changes." },
    { id:4, question:"An image recognition model is trained once on a fixed dataset of 10 million labelled photos and then deployed unchanged. Which paradigm is this?", options:["Online learning — the model sees many examples","Batch learning — the entire dataset is processed at once before deployment, and the model is then static","Reinforcement learning — the model receives feedback from users","Semi-supervised learning — most images had no labels"], correctAnswerIndex:1, explanation:"A fixed dataset, trained once, produces a static model — classic batch learning. This is appropriate when the data distribution is stable and complete." },
  ],

  "ml-lifecycle": [
    { id:1, question:"During which phase of the ML lifecycle is data leakage most likely to be introduced accidentally?", options:["Business problem definition","Feature engineering and data preprocessing — especially when transformers are fit on the full dataset before splitting","Model deployment to production","Monitoring model performance post-launch"], correctAnswerIndex:1, explanation:"The most common leakage is fitting scalers, encoders, or imputers on the entire dataset before splitting. The transformer learns test set statistics, giving the model indirect access to test data during training." },
    { id:2, question:"A model performs well in offline testing but degrades rapidly after deployment. Which lifecycle phase was likely inadequate?", options:["Data collection","Model architecture selection","Deployment and monitoring — the model was not validated under production conditions or monitored for drift after launch","Business problem definition"], correctAnswerIndex:2, explanation:"Training-serving skew (production data differing from test data) and model drift (distribution changing after deployment) are the most common causes of this pattern. Proper validation on production-like data and monitoring catch these." },
    { id:3, question:"Why does 'define the business problem' come first in the ML lifecycle, before any data collection?", options:["Because you need to write a business proposal before spending money","Because the wrong metric or objective means a technically excellent model can be commercially useless — you need to know what you're optimising before collecting data or building models","Because business teams write the feature engineering code","Because data vendors require a problem statement before selling data"], correctAnswerIndex:1, explanation:"A fraud model that maximises F1-score might still miss the business goal of minimising financial loss. If you collect data and train a model for the wrong objective, you've wasted the entire effort. Defining success first prevents this." },
  ...
  "python-for-ml": [
    { id:1, question:"Why has Python become the dominant language for machine learning, despite not being the fastest language at runtime?", options:["Python has better GPU drivers than C++","Python's simple syntax, vast ML ecosystem (NumPy, pandas, scikit-learn, PyTorch), and interactive notebooks make rapid experimentation fast — heavy computation is offloaded to C extensions anyway","Python automatically distributes computations across GPU clusters","Python was specifically designed for numerical computation"], correctAnswerIndex:1, explanation:"Python wins through iteration speed and ecosystem. Libraries like NumPy run C code under the hood, so Python's runtime slowness rarely matters for numerical ML work — the bottleneck is matrix operations, not Python loops." },
    { id:2, question:"What is a Python list comprehension and why is it preferred over a for-loop for data transformations?", options:["A list comprehension is a way to import data from external files","A concise syntax that is typically faster than an equivalent for-loop (runs optimised C internally) and makes the transformation intent immediately clear","A list comprehension creates a machine learning model from a list of parameters","They produce identical results — it is purely a style preference"], correctAnswerIndex:1, explanation:"[x**2 for x in data] vs. a for-loop that appends: the comprehension is more readable and faster because the looping happens in C rather than the Python interpreter." },
    ...
    { id:4, question:"Why should you use enumerate() rather than range(len(list)) when you need both index and value?", options:["enumerate() only works with dictionaries, not lists","enumerate() is more readable, less error-prone, and works on any iterable — you can't accidentally pass the wrong length","range(len()) always raises an IndexError","They are completely identical in every way"], correctAnswerIndex:1, explanation:"enumerate() avoids the manual index arithmetic that causes off-by-one errors, and it works on generators and other iterables that don't have a len(). It's the Pythonic standard for index-value iteration." },
  ],

  "numpy-essentials": [
    { id:1, question:"What is the fundamental performance difference between a Python list and a NumPy array?", options:["There is no performance difference — they store data identically","NumPy arrays store a single data type in contiguous memory, enabling vectorised operations in C that are 10-100x faster than Python loops on equivalent data","NumPy arrays can only store integers, while lists store any type","Python lists support mathematical operators but NumPy arrays do not"], correctAnswerIndex:1, explanation:"Python lists store Python objects (each with type metadata and reference overhead). NumPy arrays store raw C values in a contiguous block, enabling vectorised operations without Python interpreter overhead per element." },
    ...
    { id:3, question:"You have an array of shape (1000, 10). What does arr.reshape(100, 100) produce?", options:["An error — you cannot change the shape of a NumPy array","A new (100, 100) view of the same 10,000 elements rearranged according to the new shape","A (100, 100) array filled with new random values","It deletes 9,000 elements and returns a (100,) array"], correctAnswerIndex:1, explanation:"reshape is valid as long as the total number of elements is preserved: 1000×10 = 100×100 = 10,000. No data is copied — reshape returns a view with a different interpretation of the same memory." },
    { id:4, question:"What is the difference between b = arr and b = arr.copy() in NumPy?", options:["They are completely equivalent — both create independent arrays","b = arr creates a view sharing the same memory (modifying b modifies arr); arr.copy() creates an independent copy with its own memory","arr.copy() is significantly slower and should be avoided in ML code","b = arr copies values but does not copy the shape metadata"], correctAnswerIndex:1, explanation:"This is one of the most common NumPy bugs. In-place modification of what looks like a 'copy' unintentionally corrupts the original. Use .copy() or np.copy() whenever you need true independence." },
  ],
  ...
  "pandas-essentials": [
    ...
    { id:2, question:"What does df.groupby('city')['sales'].mean() return?", options:["The mean sales value across the entire dataset, ignoring the city column","A Series with one entry per unique city, containing the mean sales for customers in that city","An error — groupby requires at least two columns to be specified","Total sales per city, sorted alphabetically"], correctAnswerIndex:1, explanation:"groupby splits the DataFrame into groups by the specified column, then applies the aggregation (mean) to each group independently. This is the standard pandas pattern for computing segment-level statistics." },
    { id:3, question:"What is the risk of using df.dropna() by default on a dataset with many features?", options:["dropna() fills missing values rather than dropping, so there is no risk","If each of 50 features has a few missing values scattered independently, dropna() may remove most or all rows — dropping all rows with any missing value can silently destroy most of your dataset","dropna() only removes columns, not rows, so row count is unaffected","dropna() requires you to specify which columns to check before it runs"], correctAnswerIndex:1, explanation:"dropna() removes any row containing at least one NaN by default. With 50 features each having 5% missing values (independent), roughly 92% of rows will have at least one NaN — dropna() would discard 92% of your data silently." },
    { id:4, question:"What is the dummy variable trap that pd.get_dummies() can create, and how do you avoid it?", options:["get_dummies() creates duplicate rows rather than duplicate columns","When one-hot encoding creates n binary columns that always sum to exactly 1, they are perfectly linearly dependent — this breaks linear regression's matrix inversion. Avoid by using drop_first=True to remove one reference category","get_dummies() creates floating-point values that linear models cannot process","The trap only affects tree-based models, not linear models"], correctAnswerIndex:1, explanation:"Perfect multicollinearity (X'X is singular) makes the normal equation unsolvable. With cities [Mumbai, Delhi, Bengaluru], you need only 2 indicator columns — the third is always derivable from the other two." },
  ],

  "scikit-learn-essentials": [
    { id:1, question:"What is the primary purpose of scikit-learn's Pipeline object?", options:["To run multiple models in parallel and pick the best one","To chain preprocessing steps and a model into a single object — ensuring that transformers are fitted only on training data during cross-validation, preventing data leakage automatically","To automatically tune all hyperparameters using Bayesian search","To convert trained models to ONNX format for faster production serving"], correctAnswerIndex:1, explanation:"Without Pipeline, it's very easy to accidentally call fit_transform() on the full dataset before splitting, or to apply different transforms to train and test. Pipeline prevents this by ensuring fit() only ever touches training data." },
    { id:2, question:"Why do you call fit_transform() on X_train but only transform() on X_test?", options:["fit_transform() is available only for training data in sklearn's API","fit() learns parameters (e.g., mean and std for StandardScaler) from the data it sees. If you fit on test data, those test statistics influence your preprocessing — a form of leakage. Fit once on train, then apply (transform) to both","transform() and fit_transform() always produce identical numerical results","The test set should never be preprocessed — only raw data should be passed to the model"], correctAnswerIndex:1, explanation:"Recomputing the scaler's mean and std from test data means your preprocessing parameters 'know' about the test set. The model has indirectly seen test data before evaluation — optimistically biasing your results." },
    { id:3, question:"cross_val_score(model, X, y, cv=5, scoring='accuracy') returns what?", options:["A single float representing mean accuracy across all 5 folds","An array of 5 floats — one accuracy score per fold — showing how performance varies across different train/test splits of the data","The best model weights found across the 5 folds","A learning curve plot showing accuracy vs training set size"], correctAnswerIndex:1, explanation:"Each of the 5 folds is a different train/test split. The returned array lets you compute mean and std, which is far more informative than a single split estimate." },
  ...
  "eda": [
    { id:1, question:"A histogram of transaction amounts shows a very long right tail with a few values in the billions. What should you do before building a fraud model?", options:["Delete all transactions above ₹1 crore — they are obviously errors","Investigate whether the extreme values are genuine (wire transfers, institutional trades) or data errors. If genuine, consider log-transforming the amount feature. If errors, correct or remove them","Replace all amounts above the 99th percentile with the median","Do nothing — tree-based models handle skewed distributions without any transformation"], correctAnswerIndex:1, explanation:"Blindly removing outliers can delete genuinely important data — a billion-rupee wire transfer might itself be the fraud signal. Investigate first, then decide on transformation or removal based on what you learn." },
    { id:2, question:"A correlation matrix shows that 'total_rooms' and 'total_bedrooms' correlate at 0.97 in a housing dataset. What does this imply?", options:["Both features are essential and should definitely be kept","These features carry nearly identical information. For linear models, this creates multicollinearity — inflating coefficient standard errors and making individual feature interpretations unstable","High correlation means both features are more predictive of the target","You must remove both features together if one is correlated above 0.9"], correctAnswerIndex:1, explanation:"You might keep one, combine them into a ratio (bedrooms per room), or use Ridge regression which handles multicollinearity. Simply noting the correlation during EDA prevents you from later being confused by unstable linear model coefficients." },
    ...
    { id:4, question:"What does a Q-Q (quantile-quantile) plot reveal about a feature's distribution?", options:["The correlation between that feature and the target variable","Whether the feature's distribution matches a theoretical distribution (usually normal) — points following the diagonal indicate normality; S-shaped deviations indicate heavy tails; bowed deviations indicate skewness","The quantile rank of each individual observation in the dataset","The relationship between the feature's mean and its variance"], correctAnswerIndex:1, explanation:"Many statistical tests and linear model diagnostics assume normality. A Q-Q plot quickly reveals whether this assumption holds. A feature that looks roughly normal in a histogram might reveal heavy tails in its Q-Q plot." },
  ],

  "handling-missing-data": [
    { id:1, question:"What is the practical difference between MCAR, MAR, and MNAR missing data?", options:["They are three names for the same type of missingness — all handled the same way","MCAR: the fact that a value is missing is completely random (safe to impute). MAR: missingness depends on other observed features (imputation conditioned on those features works). MNAR: missingness depends on the missing value itself — simple imputation introduces systematic bias","They describe what percentage of values are missing: MCAR < 5%, MAR 5-30%, MNAR > 30%","MCAR is the most dangerous; MNAR is the least problematic"], correctAnswerIndex:1, explanation:"MNAR is the most dangerous: if high-income individuals leave their income field blank, imputing the mean income for those cells introduces systematic bias — your model learns the wrong relationship between income and the target." },
    ...
    { id:3, question:"A loan application dataset has 'annual_income' missing for 22% of applicants. You discover that applicants who left income blank have a 3x higher default rate. What is the best approach?", options:["Impute with the median income and proceed as normal","Create a binary 'income_not_disclosed' indicator feature AND impute the missing values — preserving both the signal in missingness and allowing the feature to contribute where it is known","Drop all rows with missing income — 22% is too high to impute","Drop the annual_income column entirely since 22% missing is unacceptable"], correctAnswerIndex:1, explanation:"The missingness itself is a 3x default rate signal — too valuable to throw away. Adding a binary indicator preserves this signal. Imputing the actual values (with median or model-based imputation) allows the income feature to contribute for the 78% where it is known." },
    { id:4, question:"A feature has 1.5% missing values, and investigation shows the missingness appears completely random. What is the most pragmatic approach?", options:["Drop the entire feature — any missing data is risky in production","Either drop the affected rows or impute with mean/median — at 1.5% random missingness, the choice has negligible impact on model performance. Focus effort on higher-impact issues","Apply MICE (Multiple Imputation by Chained Equations) — it is always the most accurate option","Rebuild the data collection pipeline before proceeding with any modelling"], correctAnswerIndex:1, explanation:"Perfect is the enemy of good. At 1.5% random missingness, sophisticated imputation methods will not measurably improve results. Drop rows or use simple imputation and spend your time on features and models that actually matter." },
  ],
  ...
  "encoding-categorical": [
    ...
    { id:2, question:"A product category feature has 2,000 unique values. What is the problem with one-hot encoding it?", options:["sklearn's OneHotEncoder cannot handle more than 100 unique categories","You create 2,000 new binary columns — dramatically increasing dimensionality, slowing training, and risking overfitting. The sparse, wide representation is inefficient","One-hot encoding converts the values to floating-point numbers that tree models cannot process","The original category column is automatically duplicated, causing problems"], correctAnswerIndex:1, explanation:"High-cardinality one-hot encoding creates very sparse, wide datasets. Better alternatives for 2,000 categories: target encoding (mean target per category), hashing trick (fixed-size hash), or learned embeddings (for neural networks)." },
    { id:3, question:"What is the dummy variable trap and how does drop_first=True in pd.get_dummies() solve it?", options:["The trap is that dummy variables always contain errors — drop_first removes the error column","When you one-hot encode n categories into n binary columns, those columns are linearly dependent (they always sum to 1). This creates perfect multicollinearity in linear models. drop_first removes one column, leaving n-1 independent indicators with the dropped category as the implicit reference","The trap is that dummy encoding is slower than label encoding — drop_first speeds it up","drop_first randomly removes one feature to reduce overfitting"], correctAnswerIndex:1, explanation:"With [is_mumbai, is_delhi, is_bengaluru] summing to 1, you can always derive one from the others. This singular matrix breaks linear algebra. Removing one column (e.g., is_bengaluru) means Bengaluru is represented by is_mumbai=0 AND is_delhi=0." },
    { id:4, question:"Target encoding replaces each category with the mean target value for that category. What leakage risk must you manage?", options:["Target encoding cannot be applied to regression problems — only binary classification","If you compute category means from the full dataset before splitting, each row's encoded value includes information from its own target label — the row literally contributes to its own encoding. Use out-of-fold target encoding or compute means only from training folds","Target encoding always leaks — the only safe approach is one-hot encoding","Leakage only occurs if you use median rather than mean for target encoding"], correctAnswerIndex:1, explanation:"Encoding row i with the mean target of its category — computed from a dataset that includes row i — means the encoding partially reflects row i's own outcome. Always compute target encoding statistics exclusively from training data, using cross-fold computation for training rows." },
  ],

  "bias-variance": [
    { id:1, question:"A decision tree achieves 99.8% training accuracy and 61% test accuracy. What is the specific problem and what are two concrete fixes?", options:["High bias (underfitting) — fix by adding more features and training longer","High variance (overfitting) — fix by limiting tree depth with max_depth and requiring a minimum number of samples per leaf with min_samples_leaf","Irreducible noise — no fix is possible","Data leakage — fix by reordering the training examples"], correctAnswerIndex:1, explanation:"The 38-point training-test gap is classic overfitting. An unconstrained decision tree memorises training examples. max_depth limits how deep the tree can grow; min_samples_leaf prevents the model from fitting individual examples." },
    { id:2, question:"Training accuracy is 64% and validation accuracy is 62% on a 10-class classification problem. Both have been stable for many epochs. What is the diagnosis?", options:["High variance — the model is memorising training data","High bias — the model is underfitting. Both train and validation are poor, meaning the model lacks the capacity to learn the patterns in this problem","Correct performance — 64% is great for 10-class problems","Irreducible error — the problem is inherently unlearnable"], correctAnswerIndex:1, explanation:"When training and validation accuracy are both poor AND similar (small gap between them), the model is underfitting. There is no evidence of overfitting (which would show a large gap). Fix: increase model complexity, add features, reduce regularisation, or train longer." },
    { id:3, question:"You have high variance. Which intervention is most likely to help: collecting more training data or reducing regularisation?", options:["Reducing regularisation — less regularisation always reduces variance","Collecting more training data — more examples make it harder for the model to memorise quirks, forcing it to learn general patterns. Reducing regularisation would increase variance, making the problem worse","Both interventions help variance equally","Neither — high variance can only be fixed by changing the model architecture"], correctAnswerIndex:1, explanation:"High variance means the model is overfitting — too sensitive to the specific training examples. More data dilutes the influence of individual noisy examples. Reducing regularisation loosens the constraints that prevent overfitting, which would worsen variance." },
  ...
  "feature-scaling": [
    { id:1, question:"A housing dataset has 'size_sq_ft' (range: 500-5000) and 'num_bathrooms' (range: 1-5). Without scaling, how does this affect a KNN model?", options:["No effect — KNN treats all features as equally important by design","The Euclidean distance calculation is completely dominated by size_sq_ft — a difference of 500 sq ft dwarfs a difference of 4 bathrooms. KNN's 'nearest neighbours' are essentially determined by size alone, ignoring bathrooms","Feature scaling makes KNN significantly slower so it is avoided in practice","KNN internally normalises all features, so external scaling is redundant"], correctAnswerIndex:1, explanation:"KNN's core operation is distance computation. A 500 sq ft difference contributes 250,000 to squared Euclidean distance; a 4-bathroom difference contributes 16. StandardScaler makes both contribute equally." },
    { id:2, question:"Why does a Random Forest NOT need feature scaling before training?", options:["Random Forests use a special internal normalisation step","Random Forests split features at threshold values (e.g., size > 2500 sq ft). Whether size is scaled to mean=0 std=1 or left as raw square footage, the relative ordering of values is unchanged — the same split is found either way","Random Forests are too fast to bother with preprocessing steps","Scaling would change the tree structure and reduce accuracy for Random Forests"], correctAnswerIndex:1, explanation:"Tree-based models (Decision Tree, Random Forest, XGBoost, LightGBM) find splits by sorting feature values and trying thresholds. Monotonic transformations like standardisation don't change which threshold minimises impurity — the trees are identical." },
    ...
    { id:4, question:"You computed mean=₹42,000 and std=₹18,000 from your training data and used them to scale X_train. What values do you use to scale X_test?", options:["Recompute fresh mean and std from X_test for the most accurate normalisation","The same mean=₹42,000 and std=₹18,000 from training — recomputing from test data would leak test information into the preprocessing pipeline","Average the training and test means: (₹42,000 + test_mean) / 2","Use mean=0 and std=1 for the test set since it represents the standardised reference"], correctAnswerIndex:1, explanation:"The scaler's parameters must be fixed from training data. If you recompute from test data, the test set's distribution influences how both sets are scaled — the model indirectly 'knows' the test set's characteristics before evaluation." },
  ],

  "feature-engineering": [
    { id:1, question:"An e-commerce dataset has a raw 'order_timestamp' column. What feature transformations might help predict order returns?", options:["Nothing — raw timestamps cannot be decomposed into useful numeric features","Extract: hour of day (late-night purchases have higher return rates), day of week, days until month-end (financial stress), time since last purchase, and whether it's a sale period — each captures a different behavioural pattern","Only convert the timestamp to Unix epoch seconds — all other decompositions lose information","Timestamps should always be dropped because they cause temporal data leakage"], correctAnswerIndex:1, explanation:"Raw timestamps are not directly predictive — but the patterns within them are. Late-night impulse purchases have higher return rates. End-of-month purchases correlate with financial stress. These patterns are invisible in the raw timestamp but become features that a model can learn from." },
    ...
    { id:3, question:"What is an interaction feature and when should you be cautious about creating them?", options:["An interaction feature is a feature that interacts with the model's hyperparameters","A new feature created by multiplying or combining two existing features, capturing their joint effect. Caution: with 50 original features, all pairwise interactions create 1,225 new features — dramatically increasing dimensionality and overfitting risk. Use domain knowledge or regularisation to select meaningful interactions","Interaction features are only valid in logistic regression, not other models","An interaction feature connects predictions from multiple models into an ensemble"], correctAnswerIndex:1, explanation:"The interaction between 'promotion_active' and 'customer_loyalty_tier' might predict conversion better than either alone. But blindly creating all 1,225 pairs from 50 features with only 500 training rows is guaranteed to overfit." },
    { id:4, question:"What makes a feature 'leaky' in the context of feature engineering, and why is it dangerous?", options:["A leaky feature has too many missing values — it 'leaks' nulls into the model","A feature uses information that would not be available at prediction time — for example, using the doctor's final diagnosis as a feature when predicting whether a patient needs urgent care, when diagnosis only exists after the decision is made. The model appears extremely accurate in testing and fails completely in production","Leaky features are those that correlate too strongly with other features","A feature is leaky if it was engineered from the test set rather than the training set"], correctAnswerIndex:1, explanation:"Target leakage is the most insidious ML mistake because it's invisible in offline evaluation. A model using a leaky feature achieves near-perfect test accuracy — and then fails immediately when deployed, because the 'future information' feature doesn't exist yet in production." },
  ],
  ...
  "feature-selection": [
    ...
    { id:2, question:"What is the difference between filter methods and wrapper methods for feature selection?", options:["Filter methods use the model's output; wrapper methods use input statistics","Filter methods assess feature relevance using statistics independent of any model (e.g., correlation, mutual information) — fast but ignores feature interactions. Wrapper methods iteratively train the actual model on different feature subsets — more accurate but orders of magnitude slower","Both methods train a model — they differ only in which metrics they use to evaluate features","Filter methods work for regression; wrapper methods only for classification"], correctAnswerIndex:1, explanation:"Mutual information filter (fast, model-agnostic) vs. Recursive Feature Elimination (trains the model multiple times to find the most impactful subset). For an initial baseline, filter is practical; for a production pipeline, wrapper methods give better results if you can afford the compute." },
    { id:3, question:"Why is Lasso regularisation described as 'embedded' feature selection?", options:["Lasso uses a built-in visualisation tool to display selected features","Feature selection is embedded into the training process itself — Lasso's L1 penalty directly drives some coefficients to exactly zero during training. There is no separate preprocessing step; the model selects and fits simultaneously","Lasso stores selected features in a separate database structure","Lasso manually removes features with low correlation before training begins"], correctAnswerIndex:1, explanation:"Filter and wrapper methods are separate preprocessing steps. Lasso integrates selection into the loss function — the penalty for nonzero weights naturally eliminates uninformative features as part of finding the minimum loss." },
    { id:4, question:"Permutation importance reports that feature X has an importance of 0.002. What does this mean practically?", options:["Feature X explains 0.2% of the total variance in the target","When feature X's values are randomly shuffled (destroying its relationship with the target), model accuracy drops by only 0.002 — meaning X contributes almost nothing to the model's predictive power","Feature X was selected in 0.2% of bootstrap samples","Feature X has a correlation of 0.002 with the target variable"], correctAnswerIndex:1, explanation:"Permutation importance answers: 'How much worse does the model perform when it can no longer use this feature?' Near-zero importance means the model performs just as well without this feature — a strong case for removing it and simplifying the model." },
  ],

  "data-visualization": [
    { id:1, question:"When would you choose a violin plot over a boxplot to visualise a distribution?", options:["Violin plots should always replace boxplots — they contain more information","When you suspect the distribution might be bimodal or have an unusual shape — a boxplot compresses all shape information into 5 summary statistics and will look similar for a bimodal and a unimodal distribution, while a violin plot shows the full KDE shape","Violin plots are only appropriate for datasets with more than 10,000 observations","When comparing fewer than 3 groups — violin plots are too visually complex for multiple comparisons"], correctAnswerIndex:1, explanation:"A distribution with two peaks (e.g., customer purchase amounts split between ₹200 budget items and ₹5,000 premium items) will show a normal-looking boxplot. The violin's KDE reveals both peaks immediately." },
    { id:2, question:"You want to visualise the correlation structure among 80 features. What is the most effective approach?", options:["Create 3,160 scatter plots of every feature pair — one for each combination","A heatmap with a diverging colour scale (blue = strong negative, white = zero, red = strong positive) — at a glance you can see clusters of correlated features, isolated features, and potential multicollinearity problems","A bar chart showing each feature's average correlation with all other features","A single correlation value averaged across all pairs — a lower number is better"], correctAnswerIndex:1, explanation:"80×80 = 6,400 correlation values. Reading a table is impossible; reading 3,160 scatter plots takes a week. A heatmap maps the full correlation matrix to colour in a single image, making patterns immediately visible." },
    { id:3, question:"A scatter plot of 'house_age' vs 'price_per_sqft' shows a U-shaped relationship — prices are high for new houses, then drop, then rise again for old historic houses. What does this mean for a linear model?", options:["A linear model is appropriate — the U-shape is just noise around a linear trend","A linear model cannot capture this relationship — it would fit a flat line through the middle. Consider adding age² as a polynomial feature or using a non-linear model like XGBoost","The U-shape means these two variables are uncorrelated and both should be dropped","Remove old houses from the dataset to make the relationship linear"], correctAnswerIndex:1, explanation:"Visualising feature-target relationships before choosing a model is exactly the point of EDA. A U-shaped relationship in a scatter plot is a direct signal that you need at minimum a quadratic term if using a linear model." },
...
Object.assign(quizData, {
  ...
  "regression-intro": [
    { id:1, question:"What distinguishes regression from classification?", options:["Regression uses trees; classification uses linear models","Regression predicts continuous numerical values; classification predicts discrete category labels","Regression is unsupervised; classification is supervised","Regression is always more accurate"], correctAnswerIndex:1, explanation:"Predicting house price (₹47 lakh) is regression. Predicting whether a house sells in under 30 days (yes/no) is classification — one of a finite set of categories." },
    { id:2, question:"A colleague reports 92% accuracy for a model predicting delivery time in minutes. Why is this suspicious?", options:["92% is too high for any real problem","Accuracy is a classification metric. For continuous numerical predictions like delivery time, you need RMSE, MAE, or R². 'Accuracy' for a regression output is undefined unless someone secretly bucketed the output","92% is an excellent result — no concern","The metric is only suspicious for imbalanced datasets"], correctAnswerIndex:1, explanation:"When someone reports accuracy for a regression problem, they have either made a conceptual error or converted the continuous output to buckets — which loses precision. Always ask which regression metric was actually computed." },
    ...
    { id:4, question:"R²=0.81 for a house price model. A stakeholder asks what this means. What do you say?", options:["The model is 81% accurate","81% of the variation in house prices across the dataset is captured by our model's features. The remaining 19% is due to factors not in the model","81% of predictions are exactly correct","The model makes 81% fewer errors than always predicting the mean"], correctAnswerIndex:1, explanation:"R² measures the proportion of variance explained. It is one of the most non-technical ways to convey model quality: 'Our model explains 81% of why prices differ between houses.'" },
  ],

  "linear-regression": [
    { id:1, question:"A residual plot shows a funnel shape — residuals are small for low fitted values and large for high ones. What problem does this indicate?", options:["This is the ideal residual plot","Heteroscedasticity — error variance is not constant. Linear regression assumes homoscedasticity. Try log-transforming the target variable to stabilise variance","Multicollinearity between features","Overfitting — the model is too confident at high values"], correctAnswerIndex:1, explanation:"Homoscedasticity (constant error variance) is a key assumption. A funnel-shaped residual plot violates it. Log-transforming the target (predicting log price instead of price) often fixes this for monetary quantities." },
    ...
    { id:3, question:"Two features correlate at 0.96. Training gives total_sq_ft coefficient=-127 and living_area=+219. How do you interpret this?", options:["These coefficients are reliable — high values mean important features","Multicollinearity has made individual coefficients unreliable and uninterpretable. A negative coefficient for total size makes no physical sense. The model may still predict well, but coefficients cannot be used for interpretation","The negative coefficient means total size is harmful to house value","Remove the feature with the larger coefficient"], correctAnswerIndex:1, explanation:"When features are highly collinear, the model can achieve the same prediction with many different coefficient combinations. The specific values become highly sensitive to noise and lose interpretability." },
    { id:4, question:"Your linear regression produces negative predictions for very old houses. What is most likely happening?", options:["Old houses are actually worth negative amounts — the model is correct","Linear regression extrapolates beyond the training distribution. If old houses were rare in training, the model extends its learned trend beyond reasonable limits. Tree-based models are less susceptible because they cannot extrapolate beyond training ranges","The model has a bug in the prediction code","You need more features to fix negative predictions"], correctAnswerIndex:1, explanation:"Linear regression is an extrapolating model — it has no built-in constraints on output range. Tree-based models return the mean of the nearest training leaf, so they cannot extrapolate beyond training data ranges." },
  ],
  ...
  "gradient-descent": [
    ...
    { id:2, question:"Training loss suddenly spikes to NaN after decreasing smoothly. What is most likely happening?", options:["The model has converged — NaN indicates zero gradient","Numerical overflow from exploding gradients or a very high learning rate. Apply gradient clipping and reduce the learning rate","NaN loss is normal for complex datasets","The training data contains NaN values"], correctAnswerIndex:1, explanation:"NaN loss is the signature of numerical overflow — gradients or weights have grown so large that floating-point arithmetic breaks down. Gradient clipping places an upper bound on gradient magnitude before each update." },
    { id:3, question:"What is the fundamental difference between batch gradient descent and SGD?", options:["Batch works for regression; SGD only for classification","Batch computes gradient using all training examples before each update (accurate but expensive). SGD updates after each single example (fast but noisy). Mini-batch (the practical default) updates after 32-256 examples","SGD uses a different loss function","Batch requires feature scaling; SGD does not"], correctAnswerIndex:1, explanation:"Mini-batch GD exploits GPU parallelism while taking frequent updates — the practical sweet spot. The noise in SGD is both a weakness (erratic path) and strength (can escape local minima)." },
    { id:4, question:"What is a saddle point and why does it cause problems in gradient descent?", options:["A saddle point is where the loss equals zero","A point where the gradient is zero but it is neither a minimum nor maximum — a minimum in some directions and maximum in others. Gradient descent stalls because the gradient is zero, giving no update signal. Momentum-based optimisers (Adam) escape saddle points more reliably","A saddle point is where learning rate becomes too small","Saddle points only exist in convex loss functions"], correctAnswerIndex:1, explanation:"In high-dimensional spaces (neural networks with millions of parameters), saddle points are far more common than local minima. Adam's adaptive learning rates and momentum help escape these flat regions." },
  ],

  "polynomial-regression": [
    { id:1, question:"A scatter plot of engine_temperature vs efficiency shows an inverted-U shape. What does this suggest?", options:["Use logistic regression since the relationship is non-monotonic","This is a degree-2 polynomial relationship. Adding temperature² as a feature lets a linear model capture this curvature — without it, linear regression would fit a flat line through the peak","Delete data points on either side and fit a line to the middle","Tree-based models cannot capture this relationship"], correctAnswerIndex:1, explanation:"The inverted-U is a quadratic relationship. A linear model cannot produce this shape. Adding the squared term gives the mathematical flexibility to capture the optimum." },
    { id:2, question:"You fit polynomial regression with degrees 1, 2, 3, 5, 10, 20 and plot training vs validation MSE. Describe what you expect.", options:["Both MSE values decrease as degree increases","Training MSE decreases consistently. Validation MSE decreases initially (capturing real curvature), then increases at high degrees (fitting noise). The optimal degree is where validation MSE is minimised","Training and validation MSE are always equal","MSE increases for all degrees above 1"], correctAnswerIndex:1, explanation:"This is the bias-variance tradeoff made concrete. Degree 1 has high bias. Degree 20 has high variance. The validation curve reveals the best tradeoff." },
    { id:3, question:"With degree-2 polynomial and 30 original features, how many total features does PolynomialFeatures produce?", options:["60 features","496 features: 30 originals + 30 squared terms + 435 pairwise interactions + 1 bias","900 features","61 features"], correctAnswerIndex:1, explanation:"Degree-2: 1 bias + 30 original + 30 squared + (30×29/2)=435 interactions = 496 total. This dimensionality explosion requires feature selection or regularisation." },
  ...
  "ridge-regression": [
    { id:1, question:"What penalty does Ridge Regression add to the MSE loss?", options:["λ × sum of absolute weights (L1)","λ × sum of squared weights (L2): Loss = MSE + λΣwᵢ²","λ × maximum weight value","λ × number of nonzero weights"], correctAnswerIndex:1, explanation:"The L2 penalty squares weights before summing. Large weights are penalised disproportionately more than small weights, creating strong pressure to keep all weights small and stable." },
    { id:2, question:"Why does Ridge specifically help when features are highly correlated?", options:["Ridge automatically removes correlated features","Standard OLS becomes numerically unstable when features are collinear (X'X is nearly singular). Ridge adds λI to X'X before inverting, making the matrix well-conditioned and producing stable coefficient estimates under multicollinearity","Ridge converts correlated features into uncorrelated ones","Ridge only helps when you have more features than examples"], correctAnswerIndex:1, explanation:"OLS solves (X'X)⁻¹X'y. With multicollinearity, X'X is nearly singular — small data changes cause huge coefficient changes. Ridge solves (X'X + λI)⁻¹X'y, which is always invertible and stable." },
    ...
    { id:4, question:"Ridge shrinks coefficients toward zero but never to exactly zero. When is this a disadvantage?", options:["Ridge should never be used when exact zeros are needed — use logistic regression","If the true model is sparse (only 10 of 500 features are relevant), Ridge keeps all 500 with small nonzero weights — hard to interpret and requiring computation of all 500 features at prediction time. Lasso (L1) drives irrelevant coefficients to exactly zero, giving a cleaner sparse model","Ridge always gives worse predictions than Lasso","Ridge should be combined with one-hot encoding to eliminate the problem"], correctAnswerIndex:1, explanation:"Ridge produces dense models — all features get weights. For interpretability or when you want to eliminate irrelevant features entirely, Lasso's sparsity property is more appropriate." },
  ],

  "lasso-regression": [
    { id:1, question:"What makes Lasso's L1 penalty produce exactly zero coefficients while Ridge's L2 only shrinks them close to zero?", options:["Lasso has a stronger penalty by convention","The L1 constraint region is diamond-shaped with corners on the axes. Optimal solutions typically land at corners where one or more coordinates are exactly zero. The L2 ball has no corners — the intersection always occurs at a point with all coordinates nonzero","Lasso applies a rounding step that zeros small coefficients","Lasso adds a post-training check that zeros coefficients below a threshold"], correctAnswerIndex:1, explanation:"This geometric intuition explains why L1 and L2 have such different behaviour. The diamond's corners create attractors where coefficients land at zero; the circle has no such attractors." },
    ...
    { id:3, question:"Lasso is applied to a genomics dataset where 50 genes are highly co-expressed. What will Lasso likely do?", options:["Lasso assigns equal weight to all 50 co-expressed genes","Lasso arbitrarily selects one gene from the co-expressed group and zeros out the other 49 — even though they are all equally informative. Ridge retains all 50 with small weights; Elastic Net selects a subset of the group","Lasso averages the 50 genes into a single representative","Lasso reports an error when features are perfectly correlated"], correctAnswerIndex:1, explanation:"Lasso's arbitrary selection among correlated features is a known weakness. Elastic Net (or Group Lasso) is preferred when correlated features should be handled as a group." },
    { id:4, question:"At λ→0 and λ→∞, what does a Lasso model predict?", options:["Set λ=1 as the universal default","At λ→0, Lasso approaches standard OLS with all features nonzero. At λ→∞, all coefficients are forced to zero and the model predicts the training mean for every input. Use cross-validation (LassoCV) to find the optimal λ between these extremes","At λ→∞, Lasso selects the single most important feature","At λ→∞, Lasso removes only the weakest feature and keeps all others"], correctAnswerIndex:1, explanation:"LassoCV efficiently searches over a path of λ values using warm starting. The two extremes illustrate the full regularisation spectrum: from pure OLS to 'ignore all features.'" },
  ],
  ...
  "classification-intro": [
    ...
    { id:2, question:"In medical screening, which type of error is more dangerous and what threshold strategy follows?", options:["False Positive — incorrectly diagnosing a healthy person","False Negative — missing a patient who has the disease. Set the threshold low (e.g., 0.2-0.3) to ensure high recall, even at the cost of lower precision. Missing a true case can be fatal; a false alarm leads to more tests, not death","Both are equally dangerous — use default threshold 0.5","Use the threshold that maximises F1-score since it balances both equally"], correctAnswerIndex:1, explanation:"In medical screening, the cost asymmetry strongly favours recall. A missed cancer is potentially fatal; an unnecessary follow-up test is an inconvenience. Set the threshold low and accept more false alarms." },
    { id:3, question:"What is a multi-label classification problem and how does it differ from multi-class?", options:["They are the same thing — just different terminology","In multi-class, each example belongs to exactly one class. In multi-label, each example can simultaneously belong to multiple classes. Example: a news article tagged as both technology and business simultaneously. This requires specialised algorithms — standard multi-class classifiers assume mutually exclusive outputs","Multi-label uses regression outputs, not classification","Multi-label is only possible with neural networks"], correctAnswerIndex:1, explanation:"Multi-label is common: image tagging (one photo can have both a dog and a cat), document classification (one document can cover multiple topics). Standard multi-class classifiers cannot be used without modification." },
    { id:4, question:"What is the decision boundary in a classification model?", options:["The accuracy threshold at which a model is acceptable","The line, curve, or hyperplane in feature space separating predicted classes. Linear classifiers produce straight line boundaries; SVMs with RBF kernel produce curved ones; tree models produce rectangular axis-aligned boundaries","The boundary between training and test data","The maximum number of features the model can handle"], correctAnswerIndex:1, explanation:"Decision boundary visualisation is one of the most powerful tools for understanding classifier behaviour. The shape of the boundary reveals what type of model is appropriate for a given dataset." },
  ],

  "logistic-regression": [
    { id:1, question:"Why does logistic regression use the sigmoid function rather than a linear output?", options:["Sigmoid speeds up gradient descent convergence","A linear combination of features can produce any real number, but probabilities must be in (0,1). The sigmoid maps any real input to (0,1), making the output interpretable as a class probability. Without it, the model could predict probabilities of -3.7 or 12.4","Sigmoid adds non-linearity allowing logistic regression to model complex boundaries","Sigmoid is computationally cheaper than alternatives"], correctAnswerIndex:1, explanation:"This is the fundamental motivation for the sigmoid. The log-odds (logit) is linear: log(p/(1-p)) = Xw. Taking the inverse logit gives the sigmoid. The model is linear in log-odds space but produces probabilities through the sigmoid transformation." },
    { id:2, question:"Logistic regression predicts default probability 0.92 for a loan applicant. At what threshold would you NOT flag this applicant?", options:["There is no threshold — 0.92 is always high-risk","Only a threshold above 0.92 would not flag this applicant. At any threshold ≤0.92 (including the standard 0.5), they are predicted as positive. Moving the threshold above 0.92 changes the prediction — but this would miss almost all defaults","The threshold must be fixed at 0.5 by regulation","The threshold only affects cases near 0.5"], correctAnswerIndex:1, explanation:"The threshold determines which probabilities become positive predictions. With p=0.92: threshold 0.5→positive, threshold 0.8→positive, threshold 0.95→negative. Raising threshold above 0.92 changes this specific prediction but at the cost of missing other likely defaults." },
    { id:3, question:"Logistic regression performs poorly and residual analysis confirms the log-odds relationship is non-linear. What is the appropriate fix?", options:["Increase regularisation — this always improves logistic regression","Add polynomial features or interaction terms to capture non-linearity within the framework, or switch to a non-linear model (Random Forest, XGBoost) that learns complex boundaries without manual feature engineering","Add more training data — logistic regression becomes non-linear with enough data","Apply PCA — it always improves logistic regression on non-linear data"], correctAnswerIndex:1, explanation:"Logistic regression is a linear classifier — its decision boundary is always a hyperplane. Non-linear boundaries require either feature engineering (explicit non-linear features) or switching to inherently non-linear models." },
  ...
  "decision-trees": [
    { id:1, question:"At a node with 60 class-A and 40 class-B examples, what is the Gini Impurity?", options:["(60-40)/100 = 0.20","1 - (0.6² + 0.4²) = 1 - (0.36 + 0.16) = 0.48","0.6 × 0.4 = 0.24","60/40 = 1.50"], correctAnswerIndex:1, explanation:"Gini = 1 - Σpᵢ² = 1 - (0.36+0.16) = 0.48. A perfectly pure node (all one class) has Gini=0. This 60/40 split is relatively impure — the tree looks for a feature split that reduces this in both child nodes." },
    { id:2, question:"What does setting min_samples_leaf=20 do, and how does it affect bias-variance?", options:["The tree can only be trained if at least 20 examples exist total","Each leaf node must contain at least 20 training examples. This prevents the tree from creating leaves for individual examples — increasing bias slightly but substantially reducing variance by preventing memorisation of small groups","The tree must make exactly 20 splits","min_samples_leaf controls features considered at each split"], correctAnswerIndex:1, explanation:"Without min_samples_leaf, a tree can create leaves for single examples (perfect training accuracy, terrible generalisation). min_samples_leaf forces the tree to generalise — each split must be supported by at least 20 examples." },
    ...
    { id:4, question:"A shallow tree (max_depth=3) consistently misclassifies the same 15% of examples regardless of training sample. What is the likely problem?", options:["Overfitting — the tree is too complex","High bias (underfitting) — the tree is too shallow to capture patterns for those 15% of examples. Consistent misclassification that doesn't depend on which training sample is used is the signature of systematic failure, not noise sensitivity","Data leakage — the 15% have information from the future","Class imbalance — the 15% are probably all minority class"], correctAnswerIndex:1, explanation:"Systematic, consistent errors that don't depend on training sample are the signature of high bias. The model is missing something fundamental. Fix: increase max_depth, add features, or try a non-linear model." },
  ],

  "naive-bayes": [
    { id:1, question:"What does naive mean in Naive Bayes, and why is this assumption made despite being wrong?", options:["The model is naive because it was designed for beginners","Naive Bayes assumes all features are conditionally independent given the class — almost always violated in real data. The assumption is made because it allows O(n×d) training instead of exponential training needed for a full joint distribution over all features","Naive means the model has no regularisation","The model ignores feature ordering in text"], correctAnswerIndex:1, explanation:"Without the independence assumption, learning P(all features|class) requires exponentially many parameters. The naive assumption makes the problem tractable: P(features|class) = ΠP(featureᵢ|class), one parameter per feature per class." },
    ...
    { id:3, question:"During training, the word cryptocurrency never appeared in spam. Without smoothing, what happens when the model sees a spam email containing it?", options:["The word is ignored and the rest is scored normally","P(cryptocurrency|spam)=0, which makes P(email|spam)=0 no matter what other spam signals are present. The email can never be classified as spam regardless of containing 100 other spam words. Laplace smoothing adds 1 to every word count, ensuring no probability is ever exactly zero","The most similar word's probability is substituted","It is treated as a missing feature and imputed with the mean spam word probability"], correctAnswerIndex:1, explanation:"This is the zero-frequency problem. One unseen word multiplies the entire class probability by zero. Laplace smoothing: add 1 to every word count so every word always has a nonzero probability." },
    { id:4, question:"You have 50,000 training examples, 200,000 vocabulary features, and need a model in 2 hours. Why is Naive Bayes an excellent starting point?", options:["Naive Bayes is always the best text classification model","Naive Bayes trains in O(n×d) time — scanning training examples once to count word frequencies per class. For 50,000 documents and 200,000 vocabulary terms, training is nearly instant. It provides a strong baseline in minutes, benchmarked against which you can justify spending time on more expensive models","Naive Bayes requires no preprocessing and works on raw text","Naive Bayes is the only algorithm handling 200,000 features"], correctAnswerIndex:1, explanation:"Naive Bayes is the standard first baseline for text classification. It trains in seconds on datasets that would take hours for gradient boosting or BERT fine-tuning." },
  ],
  ...
  "knn": [
    ...
    { id:2, question:"Feature A has values [0,1] and Feature B has values [0,100000]. Without scaling, how does KNN compute distances?", options:["KNN normalises features internally before computing distance","Euclidean distance is dominated almost entirely by Feature B — a difference of 1 in B contributes 1,000,000× more to squared distance than a difference of 1 in A. Feature A is effectively invisible","KNN uses cosine similarity by default, which is scale-invariant","KNN uses ordinal rankings, making it scale-invariant"], correctAnswerIndex:1, explanation:"This is why StandardScaler is mandatory before KNN. Without it, high-magnitude features monopolise distance calculations regardless of their actual predictive importance." },
    { id:3, question:"KNN with k=7 achieves 89% test accuracy. You increase k to 500. What do you expect?", options:["Test accuracy improves — more votes means more accuracy","Test accuracy will likely decrease. With k=500, each prediction is a majority vote from half the training set — the boundary flattens toward the dominant class everywhere. The model becomes oversmoothed, essentially predicting the global prior","Test accuracy is guaranteed to stay the same","Increasing k makes KNN equivalent to a neural network"], correctAnswerIndex:1, explanation:"k acts as a smoothness parameter. Small k → complex boundary (low bias, high variance). Large k → smooth boundary approaching the majority class baseline (high bias, low variance)." },
    { id:4, question:"Why is KNN rarely used as a primary model in production despite its simplicity?", options:["KNN is too inaccurate for production","Prediction latency scales as O(n×d) — for each new example, you compute distances to all n training examples. With 10 million training examples and 100 features, each prediction requires 1 billion distance calculations. All computation happens at prediction time, not training time","KNN cannot be serialised and saved to disk","KNN requires retraining from scratch for each new prediction"], correctAnswerIndex:1, explanation:"KNN's lazy learning is its production Achilles heel. Approximate nearest-neighbour algorithms (FAISS, Annoy, ScaNN) make KNN-like retrieval practical by trading exact accuracy for speed." },
  ],

  "svm": [
    { id:1, question:"What is the maximum margin that SVM optimises and why does it improve generalisation?", options:["SVM maximises the number of support vectors","SVM finds the hyperplane maximising the distance between the nearest examples of each class and the boundary. Larger margin means more tolerance of perturbations in new data — a new point must be further wrong before it crosses the boundary","SVM maximises the probability of correct classification on training set","SVM maximises the gap between training and test accuracy"], correctAnswerIndex:1, explanation:"The maximum margin hyperplane has the most buffer room between classes. Intuitively: it is least likely to misclassify a new point that is slightly different from training examples." },
    { id:2, question:"SVM with C=0.001 produces wide margin with 15% training error. C=1000 produces narrow margin with 0.5% training error. Which generalises better?", options:["C=1000 — lower training error always means better generalisation","Likely C=0.001 — the wide margin is more regularised and achieves a safer, more generalisable boundary. C=1000's near-zero training error suggests overfitting to specific training points. Cross-validate to confirm","They will generalise equally","C=1000 is guaranteed to generalise better by SVM theory"], correctAnswerIndex:1, explanation:"Small C = wide margin = high regularisation = accept more training mistakes. Large C = narrow margin = enforce training accuracy at the risk of overfitting." },
    { id:3, question:"You apply SVM to a dataset with 500,000 training examples. What computational challenge should you anticipate?", options:["SVM training is O(n) and handles this trivially","SVM training is O(n²) to O(n³) — the kernel matrix K where Kᵢⱼ=kernel(xᵢ,xⱼ) requires storing n² values (250 billion entries for n=500,000). Standard SVM is impractical for very large datasets; use LinearSVC (SGD-based) which scales to millions","SVM has no scalability limitations","SVM uses special algorithms at exactly 500,000 examples that are O(n log n)"], correctAnswerIndex:1, explanation:"This is SVM's key practical limitation. sklearn's SVC with RBF kernel becomes untrainable above ~100,000 examples. LinearSVC scales to millions. For large datasets with non-linear patterns, XGBoost or neural networks are more practical." },
  ...
  "train-test-split": [
    { id:1, question:"Why is it important to shuffle your dataset before splitting?", options:["Shuffling improves model accuracy by randomising the learning order","Without shuffling, any ordering (sorted by class, date, customer ID) makes the split systematically unrepresentative. All class-B examples might end up in test, or all recent data in test while old data is training. Shuffling ensures both sets are random samples from the full distribution","Shuffling is optional and has negligible effect","Shuffling is only required for neural networks"], correctAnswerIndex:1, explanation:"Real datasets are almost never randomly ordered. Medical records are often sorted by admission date. Without shuffling, you may inadvertently create a systematically biased test set." },
    ...
    { id:3, question:"You use 80/20 split and report 91.3% test accuracy. Your colleague points out the test set has only 150 examples. Should you trust this estimate?", options:["Yes — test set accuracy is always reliable","With 150 examples, the 95% confidence interval around 91.3% accuracy is approximately ±4.5%. True accuracy is likely between 86.8% and 95.8% — a 9-point wide interval. You cannot confidently distinguish a model with 87% from one with 95% accuracy. Use cross-validation for reliable estimation","Completely reliable — test set size doesn't matter","Only neural network accuracy estimates require large test sets"], correctAnswerIndex:1, explanation:"The binomial CI for accuracy p on n examples is approximately p ± 1.96√(p(1-p)/n). At n=150, p=0.913: CI ≈ 0.913 ± 0.046. Too wide to make confident model comparisons." },
    { id:4, question:"Is it ever acceptable to retrain your model on the test set after initial evaluation?", options:["Yes — using more data always improves the model","No — the test set's value comes from being genuinely held-out data simulating deployment. Retraining on it contaminates your model with test information and makes future evaluation impossible. If you want to use all data after final evaluation, retrain on train+test combined but understand you have no way to estimate generalisation performance","Yes, but only if the model achieved above 85% accuracy","Yes, as long as you do cross-validation afterward"], correctAnswerIndex:1, explanation:"The test set is sacred. Once you touch it, you have no clean held-out data left. Reserve it for one final evaluation, after all development decisions are made." },
  ],
  ...
  "cross-validation": [
    ...
    { id:2, question:"What is the difference between k-fold and stratified k-fold, and when is the latter critical?", options:["Stratified trains on more data — always better","Standard k-fold randomly assigns examples to folds without regard to class. With imbalanced classes (2% fraud), one fold might by chance have 0 fraud examples — producing undefined class-specific metrics. Stratified k-fold maintains class distribution within each fold, ensuring every fold has approximately 2% fraud examples. Critical whenever class imbalance is present","Stratified k-fold is only needed for more than 5 classes","Stratified k-fold trains on stratified samples but evaluates on random test folds"], correctAnswerIndex:1, explanation:"With severe imbalance and 5 folds, standard k-fold might create one fold with 0 positive examples. Recall and F1 are undefined for a fold with no positives. Stratified k-fold guarantees proportional class representation." },
    { id:3, question:"Model A: mean=0.87, std=0.03 on 5-fold CV. Model B: mean=0.86, std=0.08. Which do you prefer for production?", options:["Model B — 0.86 is close enough and std doesn't matter for deployment","Model A — not only higher mean, but lower std (0.03 vs 0.08) indicates far more consistent behaviour. Model B's high variance means its performance could be 0.78 or 0.94 depending on what data it encounters. Stability is a critical production requirement","Model B — higher variance means potential for higher peak performance","Neither — you need at least 10-fold CV before deciding"], correctAnswerIndex:1, explanation:"High variance in CV scores means the model is sensitive to which data it trains on. In production you cannot control what data the model encounters. Consistent moderate performance beats occasionally high but often low performance." },
    { id:4, question:"What is nested cross-validation and why is it needed for unbiased model selection?", options:["Nested CV uses two different algorithms — one inside and one outside","Nested CV uses two loops: the outer loop estimates generalisation performance, the inner loop selects hyperparameters. For outer fold k: use k as test, train on remaining. Within training data, run inner CV to pick optimal hyperparameters. Evaluate the model with those hyperparameters on outer fold k. This separates selection from evaluation — reporting inner CV score conflates both and produces optimistic estimates","Nested CV trains on nested overlapping folds to use more data","Nested CV is only needed when comparing more than 10 model types"], correctAnswerIndex:1, explanation:"Without nested CV: you tune hyperparameters using CV then report that same CV score — selection bias. Nested CV provides an unbiased estimate of the expected performance of the entire model selection procedure." },
  ],

  "overfitting-underfitting": [
    { id:1, question:"A model trained on 2019-2021 loan data achieves 93% test accuracy but only 61% when deployed in 2024. What is most likely happening?", options:["The model is overfitting — it memorised 2019-2021 training data","Distribution shift — the relationship between applicant features and default risk changed between 2019-2021 and 2024 (pre vs post-pandemic economic conditions). The model learned valid 2019-2021 patterns that no longer hold. This is concept drift, not overfitting — it generalised well within its training distribution but the distribution itself changed","The model is underfitting — add more features from 2024","The test accuracy must have been wrong — investigate leakage"], correctAnswerIndex:1, explanation:"Overfitting fails immediately on test data from the same distribution. Distribution shift produces good in-distribution generalisation but poor out-of-distribution performance. Fix: retrain with recent data, use domain adaptation, or implement monitoring and retraining triggers." },
    { id:2, question:"How do learning curves (training and validation error vs training set size) help diagnose bias vs variance?", options:["Learning curves only diagnose overfitting","High bias: both training and validation error plateau high — more data doesn't help, the model lacks capacity. High variance: training error is low but validation error is high with a large gap — both converge as you add more data. If validation error is still declining at current data size, more data will help","Learning curves show which features are most important","Learning curves are only valid for neural networks"], correctAnswerIndex:1, explanation:"Learning curves answer: what is limiting my model right now? Both plateau high = get a better model. Large gap that closes with more data = collect more examples. This guides your next action." },
    { id:3, question:"What is weight decay in neural network training and why is it equivalent to L2 regularisation?", options:["Weight decay is a training schedule that reduces the learning rate over time","At each update step, weight decay multiplies all weights by (1-ε) before adding the gradient update, shrinking them toward zero. Mathematically equivalent to adding λΣwᵢ² to the loss function — both produce the same weight update formula. Weight decay prevents any single weight from growing too large","Weight decay randomly drops weights to zero — equivalent to dropout","Weight decay and L2 regularisation produce different results in practice"], correctAnswerIndex:1, explanation:"In standard gradient descent, L2 regularisation and weight decay are exactly equivalent. They differ under adaptive optimisers (like Adam) — AdamW applies weight decay correctly without scaling it with adaptive learning rates, which is why AdamW outperforms Adam+L2 for transformers." },
  ...
  "cost-functions": [
    { id:1, question:"MSE loss is 9 for a 3-unit error and 81 for a 9-unit error. Why does this disproportionate scaling matter?", options:["It doesn't — all loss functions treat errors proportionally","The quadratic growth means large errors dominate the loss during training. If your dataset has a few genuine outliers, MSE will cause the model to heavily optimise for them at the expense of typical examples. MAE (linear scaling) treats the outlier error as 9 rather than 81 — more proportional to typical errors","MSE's quadratic scaling makes gradient computation faster","Disproportionate scaling is desirable — it ensures focus on the worst mistakes"], correctAnswerIndex:1, explanation:"The choice between MSE and MAE is a business decision about outlier treatment. Financial prediction: genuine outliers matter disproportionately → MSE appropriate. Medical measurement with noisy errors → MAE or Huber appropriate." },
    ...
    { id:4, question:"You are building a model outputting a probability distribution over 5 product categories. Which loss function is correct?", options:["MSE between predicted probabilities and one-hot labels","Categorical cross-entropy: Loss = -Σᵢ yᵢ log(pᵢ) where yᵢ=1 for the true class. This is the maximum likelihood estimate under a categorical distribution, creates proper gradients, and penalises confident wrong predictions through the log function","Binary cross-entropy applied 5 times independently","Mean Absolute Error between one-hot vector and softmax output"], correctAnswerIndex:1, explanation:"Categorical cross-entropy is standard for multi-class classification. It derives from maximum likelihood under a categorical distribution and has correct gradient direction (increases probabilities for the true class)." },
  ],

  "hyperparameter-tuning": [
    { id:1, question:"What is the difference between a model parameter and a hyperparameter?", options:["Parameters are discrete; hyperparameters are continuous","Parameters are learned from training data (weights, biases, tree split thresholds). Hyperparameters are set before training and cannot be learned by the optimisation algorithm (learning rate, n_estimators, max_depth, dropout rate)","Parameters change during training; hyperparameters are fixed at creation","There is no meaningful difference — both are adjusted during training"], correctAnswerIndex:1, explanation:"Model.fit() optimises parameters. The outer loop — trying different learning rates, depths, etc. — is hyperparameter tuning. Hyperparameters cannot be included in the loss function for gradient descent to tune along with weights." },
    ...
    { id:3, question:"What is Bayesian Optimisation and why does it outperform random search for expensive models?", options:["Bayesian Optimisation uses Bayes theorem to directly compute optimal hyperparameters","Bayesian Optimisation maintains a probabilistic surrogate model of hyperparameter-to-performance relationship (usually a Gaussian Process). After each trial it updates the surrogate and selects the next configuration most likely to improve over the current best. For expensive models (1 GPU-hour per trial), 50 intelligent evaluations outperform 50 random samples","Bayesian Optimisation is faster because it parallelises all trials simultaneously","Bayesian Optimisation always finds the global optimum; random search only finds local optima"], correctAnswerIndex:1, explanation:"The key is the surrogate model. Random search knows nothing from trial 1 when choosing trial 2. Bayesian Optimisation builds a model of which regions have produced good results and directs search toward unexplored promising regions." },
    { id:4, question:"You tune learning rate, batch size, dropout rate, num_layers, and hidden_dim with 200 iterations of random search and 5-fold CV. How many total model trainings?", options:["200 total — one per random search iteration","1,000 total: 200 configurations × 5 folds. Each configuration must be evaluated on all 5 folds. For a neural network training in 4 hours per fold, this is 4,000 hours — illustrating why efficient tuning methods matter enormously for expensive models","200 trainings — CV is run once at the end to validate the best configuration","25 trainings — random search subsamples from the 5 folds automatically"], correctAnswerIndex:1, explanation:"This arithmetic makes the cost of hyperparameter tuning concrete. 200 configurations × 5 folds = 1,000 trainings. For deep learning this can be prohibitive. Bayesian optimisation with 30-50 configurations × 3-fold CV = 90-150 trainings, often finding similar or better results." },
  ],
  ...
  "grid-random-search": [
    ...
    { id:2, question:"Bergstra & Bengio (2012) showed random search outperforms grid search when only 2 of 6 hyperparameters actually matter. Why?", options:["Grid search wastes evaluations on non-influential hyperparameters","Grid search tries a fixed grid — if only 2 parameters matter, grid search wastes 4/6 of its budget varying unimportant parameters while keeping important ones fixed across groups. Random search samples all 6 parameters independently each trial, giving every trial a unique value for the 2 important parameters — far more efficient for finding the good region in the important dimensions","Random search uses a more powerful search algorithm","Grid search cannot search continuous hyperparameter spaces"], correctAnswerIndex:1, explanation:"If learning_rate and max_depth matter but num_trees, subsample, colsample, and reg_lambda don't, grid search wastes 4/6 of its diversity budget on irrelevant parameters. Random search automatically uses all budget more effectively on the important dimensions." },
    { id:3, question:"When does grid search have a genuine advantage over random search?", options:["Grid search is never better — always use random search","Grid search guarantees complete coverage of a small, discrete search space. For example: kernel=['linear','rbf','poly'] × C=[0.1, 1, 10] — 9 combinations, each meaningful, all equally worth evaluating. For small discrete spaces where all combinations are plausible, grid search's exhaustiveness is valuable. For large or continuous spaces, random search wins","Grid search is better when training is very slow","Grid search is better for neural networks; random search for tree models"], correctAnswerIndex:1, explanation:"The crossover point: if the search space has fewer than ~20-30 total meaningful combinations, grid search's exhaustiveness provides value. Above that, random search's efficiency dominates." },
    { id:4, question:"What is Halving Search (HalvingGridSearchCV in sklearn) and why is it more efficient?", options:["Halving search evaluates each configuration on half the dataset","Halving search starts with many configurations evaluated on a small budget (small training set or few CV folds). The worst half are eliminated. Survivors are evaluated with double the budget. This continues until one configuration remains. Poor configurations are identified cheaply and eliminated early; compute is concentrated on promising configurations. Often achieves similar results to standard search with 5-10x less total compute","Halving search halves the hyperparameter search space at each step","Halving search terminates after half the normal iterations"], correctAnswerIndex:1, explanation:"Successive halving is a bandit-style algorithm: spend a little to identify and eliminate bad options, then spend more on the remaining promising ones. sklearn implements HalvingGridSearchCV and HalvingRandomSearchCV." },
  ],

  "confusion-matrix": [
    { id:1, question:"TP=45, FP=15, FN=5, TN=935. Calculate precision and recall and explain both for a medical context.", options:["Precision=45%, Recall=5%","Precision=75%: of 60 flagged patients, 75% truly have the disease. Recall=90%: of 50 true cases, 90% were detected. For medical screening, recall (catching 90% of true cases) is the priority — the 10% missed cases are the most dangerous outcome","Precision=93.5%, Recall=90%","Precision=90%, Recall=75%"], correctAnswerIndex:1, explanation:"Precision=45/(45+15)=75%. Recall=45/(45+5)=90%. In disease screening: FN (missing a sick patient) is catastrophic; FP (unnecessary follow-up) is manageable. High recall is the right priority." },
    { id:2, question:"A model predicts not-fraud for every transaction on a dataset with 0.3% fraud rate. What does its confusion matrix look like?", options:["TP=0, FP=100, FN=0, TN=900","TP=0, FP=0, FN=3000, TN=997000 (assuming 1M transactions). Accuracy=99.7% — looks excellent while the model is completely useless. Recall=0/3000=0%","TP=997, FP=3, FN=0, TN=0","TP=3000, FP=0, FN=997000, TN=0"], correctAnswerIndex:1, explanation:"This confusion matrix exposes what accuracy hides: Recall=0% and F1≈0. A model achieving 99.7% accuracy is completely worthless for its stated purpose — catching fraud." },
    { id:3, question:"Users are complaining that legitimate emails go to spam (false positives). What change do you make and what is the consequence?", options:["Add more training data — FP reduction always requires more data","Raise the classification threshold above 0.5 — predict spam only when the model is more confident. This reduces FP (fewer legitimate emails flagged) but increases FN (more actual spam gets through). The confusion matrix shifts: FP↓, FN↑, TP↓, TN↑. Precision increases; recall decreases","Apply SMOTE to the legitimate class","Switch to a different algorithm"], correctAnswerIndex:1, explanation:"Threshold adjustment is the most direct lever for controlling the FP-FN tradeoff without retraining. Raising the threshold requires higher model confidence to classify as positive — fewer false positives, but some real spam also escapes." },
  ...
  "roc-auc": [
    { id:1, question:"What happens as you move the classification threshold from 1.0 to 0.0, and how does this create the ROC curve?", options:["The ROC curve plots training vs test accuracy as threshold changes","At threshold=1.0: nobody predicted positive → TPR=0, FPR=0. As threshold decreases: more examples predicted positive. True positives are captured (TPR increases — up y-axis). Some negatives are also incorrectly captured (FPR increases — right along x-axis). A perfect model captures all positives before any negatives. A random model captures both at equal rates (diagonal line). AUC summarises the model's ability to rank positives above negatives across all thresholds","The ROC curve shows how accuracy changes with different learning rates","At threshold=0.5 the ROC curve reaches its optimal point"], correctAnswerIndex:1, explanation:"Understanding the ROC curve as a threshold sweep is crucial. The curve encodes the model's ranking ability. AUC=0.9 means: if you pick a random positive and random negative, the model ranks the positive higher 90% of the time." },
    { id:2, question:"Model A AUC=0.91, Model B AUC=0.89. What additional analysis is needed before concluding A is better?", options:["No analysis — higher AUC always indicates a better model","Bootstrap confidence intervals on both AUC estimates. With 500 test examples, the 95% CI for AUC is approximately ±0.04. If CIs overlap substantially, the difference is not statistically significant. Also consider: whether AUC is the right metric for this application, computational cost, and performance at the specific threshold you will use in production","The models must be evaluated on separate test sets","Always prefer the model with higher AUC — statistical testing is only for academic papers"], correctAnswerIndex:1, explanation:"The difference of 0.02 in AUC is meaningful only if it exceeds estimation uncertainty. With small test sets, this difference is not statistically significant and might reverse on a different test sample." },
    ...
    { id:4, question:"What does AUC=0.5 mean and how do you verify this is not a bug?", options:["AUC=0.5 means the model is 50% accurate — acceptable for hard problems","AUC=0.5 means predictions are no better than random at ranking positives above negatives. Verification: (1) Check for label inversion — if AUC=0.5 with normal labels gives AUC=0.95 with inverted labels, your labels are reversed. (2) Check if predictions have zero variance (model always predicts the same value). (3) Verify feature-label correspondence was maintained during preprocessing","AUC=0.5 is impossible — AUC is always between 0.6 and 1.0","AUC=0.5 means the model has converged to a saddle point — adjust the learning rate"], correctAnswerIndex:1, explanation:"AUC=0.5 is the most suspicious result. The most common cause: accidentally inverted labels (0 and 1 swapped during encoding). Second: all predictions are identical (zero variance). Third: features and labels got misaligned during data processing." },
  ],
});
...
Object.assign(quizData, {
  ...
  "arima": [
    ...
    { id:3, question:"ACF cuts off sharply after lag 2, while PACF decays gradually. What ARIMA order does this suggest?", options:["ARIMA(p,d,0) — pure AR","ARIMA(0,d,2) — pure MA(2). Sharp ACF cutoff and gradual PACF decay is the MA signature","ARIMA(2,d,2)","No ARIMA fits this pattern"], correctAnswerIndex:1, explanation:"ACF cuts off at lag q → MA(q). PACF cuts off at lag p → AR(p). Gradual PACF and ACF cutting off at lag 2 → MA(2) process." },
    { id:4, question:"How do you check if ARIMA residuals indicate a good fit?", options:["Check if residuals match the original series pattern","Good residuals should be white noise: ACF of residuals shows no significant autocorrelation at any lag, and Ljung-Box test p-value > 0.05","Residuals should perfectly predict the original values","Residuals should have the same variance as the original series"], correctAnswerIndex:1, explanation:"If residuals contain autocorrelation, the model hasn't fully captured the time series structure. White noise residuals confirm the model explains all systematic variation." },
  ],
  ...
  "moving-average": [
    ...
    { id:2, question:"SMA-7 is applied to daily sales data. What is the main limitation?", options:["SMA-7 cannot handle daily data","SMA-7 produces no forecast — it only smooths historical data. Additionally, it lags real changes by approximately 3 days (half the window)","SMA-7 requires stationarity","SMA-7 is computationally too expensive for 7 points"], correctAnswerIndex:1, explanation:"Moving averages are smoothing tools, not forecasting models. The lag is inherent — averaging the last 7 points means the SMA always lags the true series by roughly window/2 days." },
    { id:3, question:"What is a golden cross in the context of moving averages?", options:["A mathematical artifact indicating data corruption","When a short-period MA (e.g., 50-day) crosses above a long-period MA (e.g., 200-day) — used as a bullish signal in technical analysis suggesting upward momentum","The point where SMA and EMA values converge","When the moving average equals the original data"], correctAnswerIndex:1, explanation:"The golden cross signals that short-term average has risen above long-term average, suggesting a recent upward trend strong enough to reverse the longer-term direction." },
    { id:4, question:"Why should you NOT use future data points when computing a moving average for a real-time forecasting system?", options:["Future data doesn't affect the average mathematically","Using future values creates look-ahead bias — the system appears to know future values it would not have in production. This inflates metrics but the system fails immediately when deployed","Future data improves accuracy","Moving averages don't use future data by definition"], correctAnswerIndex:1, explanation:"A centered moving average uses future data that doesn't exist in real-time. Always use trailing-only (causal) moving averages for live forecasting systems." },
  ],

  "exponential-smoothing": [
    { id:1, question:"What does the smoothing parameter alpha control in Simple Exponential Smoothing?", options:["The window size — higher alpha uses more historical data","The weight given to the most recent observation vs smoothed history. Alpha close to 1: high reactivity (noisy). Alpha close to 0: slow adaptation (stable)","The number of exponential components","The forecast horizon"], correctAnswerIndex:1, explanation:"F_t = alpha x Y_{t-1} + (1-alpha) x F_{t-1}. Alpha=0.9 means recent data dominates; alpha=0.1 means historical average dominates. Choose based on how quickly the series actually changes." },
    { id:2, question:"Simple Exponential Smoothing produces constant forecasts for all future periods. Why is this a problem?", options:["SES cannot produce future forecasts","The SES forecast for all future periods equals the last smoothed value — it projects a flat line. This fails for series with trend or seasonality. Holt's method adds trend; Holt-Winters adds both trend and seasonality","SES forecasts decline exponentially","SES produces a different forecast for each future period"], correctAnswerIndex:1, explanation:"SES has no trend component. A growing business would be forecast to stay constant forever. Choose the variant matching your series structure." },
    { id:3, question:"When would Holt-Winters multiplicative seasonality be chosen over additive?", options:["When the series has no seasonality","When seasonal fluctuations grow proportionally with the level — e.g., December sales always 20% above trend (not a fixed amount). Additive assumes a fixed seasonal amount regardless of level","Multiplicative is always more accurate","When the seasonal period exceeds 12 months"], correctAnswerIndex:1, explanation:"Additive: seasonal effect = fixed amount. Multiplicative: seasonal effect = fixed proportion. If seasonal patterns grow with the series level, multiplicative produces more accurate forecasts." },
  ...
  "forecasting-basics": [
    { id:1, question:"What is the difference between in-sample fit and out-of-sample forecast accuracy?", options:["They are the same — good in-sample fit always means good forecasts","In-sample measures how well the model fits historical training data; out-of-sample measures performance on future data never seen. A model can have excellent in-sample fit (memorising noise) and terrible out-of-sample performance","Out-of-sample is always better","In-sample fit is more important for operational systems"], correctAnswerIndex:1, explanation:"A complex ARIMA might fit every historical wiggle perfectly but produce poor forecasts. Always evaluate on a held-out future period the model never saw." },
    { id:2, question:"What is the naive forecast and why is it a critical baseline?", options:["Predicting the series mean for all future periods","The naive forecast predicts the next value equals the last observed value (stationary) or the same period last year (seasonal). If your complex model doesn't beat this, it has learned nothing useful from the data structure","A forecast without any data","A forecast using only the last 3 observations"], correctAnswerIndex:1, explanation:"The seasonal naive model is surprisingly hard to beat for many business series. If your model beats seasonal naive, it is demonstrably adding value." },
    ...
    { id:4, question:"What is a prediction interval and how does it differ from a confidence interval?", options:["They are identical — just different terminology","A prediction interval gives a range for an individual future observation (accounting for model uncertainty AND inherent series variability). A confidence interval gives a range for the mean forecast (model uncertainty only). For operational planning, prediction intervals are what you need","Prediction intervals only apply to point forecasts","Confidence intervals are always wider"], correctAnswerIndex:1, explanation:"For inventory planning, you need the range of likely demand for a specific future day — that is a prediction interval. The wider prediction interval includes both model uncertainty and irreducible variability." },
  ],

  "semi-supervised": [
    { id:1, question:"What problem does semi-supervised learning solve?", options:["Difficulty of training without any data","The high cost of labelling — when you have abundant unlabelled data but few labelled examples, semi-supervised learning leverages unlabelled data structure to improve beyond what labelled data alone achieves","Inability of supervised models to handle continuous features","Overfitting in neural networks"], correctAnswerIndex:1, explanation:"Labels are expensive — expert annotation takes time and money. Unlabelled data is cheap. Methods like self-training, label propagation, and pseudo-labelling bridge this gap." },
    ...
    { id:3, question:"What is the cluster assumption in semi-supervised learning?", options:["All clusters contain the same number of examples","Points in the same cluster are likely to share the same label — decision boundaries should pass through low-density regions. This allows unlabelled data cluster structure to improve classification boundaries","Clusters must be spherical","Number of clusters equals number of classes"], correctAnswerIndex:1, explanation:"If this assumption holds, the unlabelled data cluster structure tells you where to draw class boundaries. If it doesn't hold, semi-supervised learning can actually hurt performance." },
    { id:4, question:"In NLP, BERT uses self-supervised pretraining. How does this relate to semi-supervised learning?", options:["BERT is standard supervised — uses billions of labelled examples","BERT's pretraining (predicting masked words in unlabelled text) is self-supervised — labels are derived from the data itself. A form of semi-supervised learning where labelling is automatic. The representations are then fine-tuned with a small amount of task-specific labelled data","BERT is entirely unsupervised","BERT requires labelled data during pretraining"], correctAnswerIndex:1, explanation:"Pretraining on massive unlabelled corpora (self-supervised) + fine-tuning on small labelled datasets achieves what previously required millions of labelled examples — the core semi-supervised paradigm at massive scale." },
  ],
  ...
  "online-learning": [
    ...
    { id:2, question:"What is out-of-core learning and why does online learning solve it?", options:["Learning outside normal business hours","When the dataset is too large for RAM. Online learning processes data in small chunks from disk, updating the model with each chunk without ever loading the full dataset into memory","Learning from data stored outside the main database","Cloud-based learning"], correctAnswerIndex:1, explanation:"A 10TB dataset of click events cannot fit in RAM. Online learning processes it in small chunks. River (formerly scikit-multiflow) and sklearn's partial_fit() enable this." },
    { id:3, question:"What is catastrophic forgetting in online learning?", options:["The model crashes when processing a large batch","Online updates on new data cause the model to forget previously learned patterns — adapts too quickly, losing knowledge of older patterns still relevant","A memory overflow error","When learning rate becomes too small"], correctAnswerIndex:1, explanation:"High learning rate adapts quickly but forgets. Low learning rate retains old knowledge but adapts slowly. Elastic Weight Consolidation and memory replay mitigate this." },
    { id:4, question:"How do you evaluate an online learning model when new labelled examples arrive continuously?", options:["Evaluate once on a fixed test set at the end","Use prequential evaluation (test-then-train): for each new example, predict first, then update with the true label. This gives a running evaluation that accurately reflects performance at each point in time without a separate held-out set","Freeze the model periodically and evaluate on a batch","Online models cannot be evaluated"], correctAnswerIndex:1, explanation:"Prequential evaluation avoids look-ahead bias and gives a realistic picture of performance over time, including adaptation periods after concept drift." },
  ],

  "reinforcement-learning-adv": [
    { id:1, question:"What is the Q-function and what does Q(s,a) represent?", options:["Q is the quality of training data","Q(s,a) is the expected total future reward from taking action a in state s and then following the optimal policy. Q-Learning updates Q values using the Bellman equation, converging to true Q-star given enough exploration","Q is the probability of a state occurring","Q(s,a) is the immediate reward for action a in state s"], correctAnswerIndex:1, explanation:"The distinction between immediate reward and expected cumulative reward is critical. An action with small immediate reward but high-reward future state has a high Q value." },
    { id:2, question:"What is epsilon-greedy and how does it address exploration-exploitation?", options:["A tradeoff between training speed and accuracy","Randomly explores with probability epsilon and exploits (takes the known best action) with probability 1-epsilon. Typically epsilon decays over training — high exploration early, more exploitation later as Q values stabilise","A method for selecting hyperparameters","A learning rate decay schedule"], correctAnswerIndex:1, explanation:"Early in training epsilon is high — the agent doesn't know enough to exploit. As Q values stabilise, epsilon decreases — more exploitation. The decay schedule itself is an important hyperparameter." },
    { id:3, question:"What problem does a replay buffer solve in Deep Q-Networks (DQN)?", options:["It prevents the network from memorising individual experiences","Neural networks assume IID training examples. In RL, consecutive experiences are highly correlated. A replay buffer stores past experiences and samples random mini-batches — breaking temporal correlation and stabilising training","A replay buffer increases the number of training episodes","It stores the best policy found so far"], correctAnswerIndex:1, explanation:"Without replay buffer: training on correlated states causes catastrophic overfitting to the current trajectory. With it: random sampling breaks correlation, making training stable." },
  ...
  "multi-armed-bandits": [
    { id:1, question:"What is the exploration-exploitation tradeoff in the multi-armed bandit problem?", options:["Choosing between multiple computers","Each arm (option) has an unknown reward distribution. Exploitation: pull the arm with highest estimated reward. Exploration: try less-known arms for better estimates. Too much exploitation: miss a better arm. Too much exploration: waste pulls on poor arms","Choosing between different ML algorithms","Tradeoff between accuracy and speed"], correctAnswerIndex:1, explanation:"The classic real-world bandit problem is A/B testing: exploit the currently best website version or explore new variants? Bandit algorithms adaptively shift traffic toward better-performing variants during the test." },
    { id:2, question:"What is UCB (Upper Confidence Bound) and how does it balance exploration-exploitation?", options:["UCB always picks the arm with highest observed average reward","UCB selects the arm with the highest upper confidence bound: estimated mean + confidence bonus. The bonus is larger for less-tried arms — naturally directing exploration toward uncertain arms. The bonus shrinks as each arm is tried more","UCB randomly selects arms proportional to observed rewards","UCB is epsilon-greedy with decreasing epsilon"], correctAnswerIndex:1, explanation:"UCB has optimism in the face of uncertainty — it tries uncertain arms because they might be better than they appear. As an arm is explored more, its confidence interval shrinks and it competes on mean alone." },
    ...
    { id:4, question:"What is Thompson Sampling and what is its key advantage over epsilon-greedy?", options:["Thompson Sampling uses gradient descent to select arms","Thompson Sampling maintains a probability distribution over expected reward per arm. It selects an arm by sampling from each distribution and picking the highest sample — naturally exploring uncertain arms (wide distributions) while exploiting consistent performers","Thompson Sampling explores equally across all arms","Thompson Sampling requires knowing the optimal arm in advance"], correctAnswerIndex:1, explanation:"Epsilon-greedy explores uninformedly — it occasionally tries random arms. Thompson Sampling explores informedly — arms with uncertain distributions are more likely to produce high samples, directing exploration where most valuable." },
  ],
  ...
  "neural-networks": [
    ...
    { id:2, question:"What job does a bias perform in a neuron?", options:["It stores the target label","It adds an adjustable offset to the weighted sum before activation","It replaces every input weight","It calculates the final accuracy"], correctAnswerIndex:1, explanation:"A neuron commonly computes z = Σwx + b. The bias b lets the activation shift instead of being forced through the origin." },
    { id:3, question:"After a forward pass produces a prediction, why is a loss function used?", options:["To add more hidden layers","To measure how the prediction differs from the target with one training objective","To shuffle the training examples","To save the model"], correctAnswerIndex:1, explanation:"The loss converts the prediction-versus-target error into a number that training tries to reduce." },
    { id:4, question:"Which statement correctly separates backpropagation from the optimizer?", options:["Both only make predictions","Backpropagation computes gradients; the optimizer uses them to update weights and biases","The optimizer computes labels; backpropagation loads data","Backpropagation saves the model; the optimizer evaluates it"], correctAnswerIndex:1, explanation:"Backpropagation tells us how the loss changes with each parameter. The optimizer turns those gradients into parameter updates." },
  ],

  "deep-learning-intro": [
    { id:1, question:"Which combination helped modern deep learning accelerate around 2012?", options:["Better algorithms, more researchers, and government funding","Larger labelled datasets, faster parallel hardware such as GPUs, and improved architectures and training methods working together","Faster internet, cloud storage, and Python","Better theory, interpretability research, and academic publishing"], correctAnswerIndex:1, explanation:"AlexNet's 2012 ImageNet result is one prominent example: it combined a large labelled dataset, GPU computation, and an effective convolutional architecture. The relative contribution of each factor varies by problem, and neural-network research began decades earlier." },
    ...
    { id:3, question:"What is transfer learning in deep learning and why can it help?", options:["Moving a trained model from one computer to another","Using a model pretrained on a large dataset as a starting point. Its learned features can be adapted to a related task, which may reduce labelled-data and compute needs","Transferring hyperparameters from one model to another","Loading weights from a different task always improves performance"], correctAnswerIndex:1, explanation:"A model pretrained on a large image collection may contain reusable visual features. Fine-tuning can be effective when the source and target tasks are sufficiently related, but the benefit depends on domain match, data quality, and the fine-tuning procedure." },
    { id:4, question:"What is the dying ReLU problem and how can Leaky ReLU help?", options:["ReLU neurons die from numerical overflow","A ReLU unit can keep receiving negative pre-activations, so its local derivative is zero and its incoming weights may stop changing. Leaky ReLU uses a small chosen slope on the negative side, which can preserve a gradient there","Dying ReLU means the network converges too quickly","Leaky ReLU prevents gradients from becoming too large"], correctAnswerIndex:1, explanation:"For ReLU, f(x)=max(0,x), so the derivative is zero when x<0. Leaky ReLU instead uses f(x)=x for x>0 and f(x)=alpha*x otherwise, where alpha is a small hyperparameter such as 0.01. This reduces, but does not guarantee elimination of, inactive units." },
  ],
...
Object.assign(quizData, {
  ...
  "random-forest": [
    { id:1, question:"Why does a Random Forest typically outperform a single Decision Tree?", options:["It trains on more data","100 trees trained on different bootstrap samples with random feature subsets — errors are partially independent, so averaging substantially reduces variance","It uses a better loss function","It limits depth to 3 levels"], correctAnswerIndex:1, explanation:"Bootstrap sampling creates different errors per tree. Feature randomness prevents all trees relying on the same dominant feature. Independent errors cancel when averaged." },
    { id:2, question:"What is the Out-of-Bag (OOB) error?", options:["Error on examples the model refused to classify","Validation error estimated using examples not in each tree's bootstrap sample (~37% per tree) — a free cross-validation estimate without a separate split","Error rate on the test set","Error from random feature subsets"], correctAnswerIndex:1, explanation:"Bootstrap selects ~63% unique examples per tree. The remaining ~37% are OOB — they serve as free validation. Averaging OOB errors across all trees gives a reliable performance estimate." },
    ...
    { id:4, question:"How does increasing n_estimators affect bias and variance in Random Forest?", options:["High bias — adding more trees reduces bias","High variance per individual tree — ensemble averaging reduces variance. More trees further reduces variance with diminishing returns; does not increase bias","Both — ensemble methods suffer from both","n_estimators has no effect — only max_depth matters"], correctAnswerIndex:1, explanation:"Individual trees are high-variance. Random Forest reduces variance through averaging. More trees → less variance with diminishing returns after ~100-200 trees. Beyond ~500 trees, improvements are negligible." },
  ],

  "bagging": [
    { id:1, question:"What does bootstrap mean in Bootstrap Aggregating (Bagging)?", options:["Using a pre-trained model as a starting point","Sampling the training set with replacement — each bootstrap sample contains the same number of examples but with duplicates, missing roughly 37% of unique examples","Loading model weights from a saved file","Initialising weights randomly"], correctAnswerIndex:1, explanation:"Sampling with replacement creates diverse training sets for each base learner, making their errors partially independent — the key to why averaging works." },
    { id:2, question:"Bagging primarily reduces which component of model error?", options:["Irreducible noise","Bias","Variance — averaging multiple models trained on different data samples makes their random errors cancel out","Both bias and variance equally"], correctAnswerIndex:2, explanation:"Bagging works because each model's variance errors differ (due to different bootstrap samples) while systematic errors (bias) are similar. Averaging uncorrelated errors reduces variance; systematic errors remain." },
    { id:3, question:"Why does Bagging work less effectively when base models are highly correlated?", options:["Correlated models take longer to train","If models are highly correlated, their errors are the same — averaging identical errors doesn't reduce them. Decorrelation (feature randomness in Random Forest) is needed for averaging to be effective","Correlation causes numerical instability","Correlated models always overfit"], correctAnswerIndex:1, explanation:"The mathematical benefit of averaging comes from error independence. If model A and B make the same errors, averaging gives the same answer with no variance reduction." },
    { id:4, question:"Can Bagging be applied to reduce variance of logistic regression?", options:["No — Bagging only works with decision trees","Yes — training multiple logistic regression models on bootstrap samples and averaging probability outputs reduces variance, though the gain is smaller than with high-variance learners like trees","Yes — but only with datasets over 10,000 examples","No — logistic regression already has zero variance"], correctAnswerIndex:1, explanation:"Bagging is model-agnostic. However, logistic regression is relatively low-variance (simple linear boundary), so the variance reduction from bagging is smaller than for trees." },
  ],
  ...
  "boosting": [
    ...
    { id:2, question:"Boosting primarily reduces which component of error compared to Bagging?", options:["Boosting reduces variance; Bagging reduces bias","Boosting reduces bias — each iteration corrects systematic errors. Bagging reduces variance. This is why Boosting can turn weak learners into powerful models","They reduce the same error type","Boosting reduces both by more than Bagging"], correctAnswerIndex:1, explanation:"A weak learner has high bias. Boosting chains them — each model reduces remaining bias. Bagging of the same weak learner reduces variance but cannot overcome fundamental high-bias." },
    { id:3, question:"Why is Boosting more prone to overfitting than Bagging?", options:["Boosting uses higher-depth trees by default","Relentlessly focusing on hard examples means eventually memorising genuine noise — mislabelled data gets continuously up-weighted. Use early stopping, shrinkage, and subsampling to mitigate","Bagging has built-in regularisation that Boosting lacks","Boosting uses gradient descent which inherently overfits"], correctAnswerIndex:1, explanation:"Boosting's strength is its weakness: relentlessly focusing on hard examples eventually memorises noise. Early stopping when validation loss starts increasing is essential." },
    { id:4, question:"The learning rate in Gradient Boosting is called shrinkage. What does it shrink?", options:["The size of the trees","The contribution of each new tree — learning rate 0.01 means each tree contributes only 1% of its full prediction, requiring more trees but better generalisation","The number of features per split","The training dataset size per iteration"], correctAnswerIndex:1, explanation:"Low learning rate + more trees (with early stopping) is generally the best configuration. Small steps create smoother, more stable corrections." },
  ],

  "adaboost": [
    { id:1, question:"How does AdaBoost adjust training data between iterations?", options:["It removes correctly classified examples","It increases the weight of misclassified examples and decreases correctly classified ones — forcing the next classifier to focus on current mistakes","It randomly shuffles training data","It adds noise to misclassified examples"], correctAnswerIndex:1, explanation:"After each weak learner, examples it got wrong get higher weights — emphasised in the next iteration. The final prediction is a weighted vote where better classifiers get more say." },
    { id:2, question:"What determines each classifier's vote weight in AdaBoost?", options:["All classifiers get equal weight","Classifiers with lower error rates get higher vote weights — alpha = 0.5 x ln((1-error)/error). A near-perfect classifier gets large weight; one barely better than random gets near-zero weight","Vote weight is determined by training order","Vote weight is determined by classifier size"], correctAnswerIndex:1, explanation:"At error=0.01, alpha is approximately 2.3 (high weight). At error=0.49, alpha is approximately 0.025 (near zero). Only meaningful classifiers substantially influence the final prediction." },
    { id:3, question:"What is a weak learner in AdaBoost and why are decision stumps most common?", options:["A model with accuracy below 50%","Any model performing slightly better than random (error < 50%). Decision stumps (single split) are popular: fast, diverse, and predictably weak — AdaBoost converts many simple rules into a strong combined classifier","A weak learner must be a linear model","Randomly initialised untrained models"], correctAnswerIndex:1, explanation:"AdaBoost only requires each weak learner to do better than chance. Decision stumps satisfy this cheaply. Using complex base learners makes AdaBoost slower without benefit." },
  ...
  "gradient-boosting": [
    { id:1, question:"What does each new tree in Gradient Boosting actually learn to predict?", options:["The original target from scratch","The residual errors of the current ensemble — how far off the current prediction is. Each tree is a model of the previous model's mistakes","A random subset of training examples","The probability each example was classified correctly"], correctAnswerIndex:1, explanation:"Tree T+1 predicts the correction needed to the current prediction. After 100 trees, the final prediction is the baseline plus 100 small corrections." },
    { id:2, question:"How does learning rate interact with n_estimators in Gradient Boosting?", options:["Learning rate determines tree depth","Smaller rate means each tree contributes less, requiring more trees (higher n_estimators) but usually better generalisation. High learning rate with few trees often overfits","They are independent","Learning rate should always equal 1/n_estimators"], correctAnswerIndex:1, explanation:"Classic strategy: learning_rate=0.01 (small) with early stopping to find optimal n_estimators. This learn-slowly approach is consistently strong." },
    ...
    { id:4, question:"Training loss keeps decreasing but validation loss starts increasing after 200 trees. What should you do?", options:["Continue training — the model has not converged","Apply early stopping — halt at the number of trees where validation loss was minimum. The model is overfitting after that point","Increase the learning rate","Reduce the number of features"], correctAnswerIndex:1, explanation:"This is textbook overfitting in Gradient Boosting. Each additional tree memorises training noise. Early stopping is the standard solution." },
  ],

  "xgboost": [
    { id:1, question:"What makes XGBoost significantly faster than sklearn's Gradient Boosting?", options:["XGBoost uses a different language","A column block data structure that sorts feature values once and reuses them across splits — enabling parallelised split finding, cache-aware computation, and out-of-core processing for large datasets","XGBoost trains fewer trees by default","XGBoost uses 32-bit arithmetic"], correctAnswerIndex:1, explanation:"sklearn's GradientBoostingClassifier builds trees sequentially with no parallelisation. XGBoost parallelises feature split evaluation within each tree level." },
    ...
    { id:3, question:"When do you use scale_pos_weight in XGBoost?", options:["To scale all positive prediction values","For imbalanced classification — set to the ratio of negative to positive examples (e.g., 99 for 1% positive rate) — XGBoost penalises missing positive examples proportionally more","It controls the learning rate for positive examples","It scales weight initialisation"], correctAnswerIndex:1, explanation:"Setting scale_pos_weight=99 tells XGBoost to treat each positive example as 99 times more important — equivalent to oversampling positives without duplicating data." },
    { id:4, question:"What does the subsample parameter do in XGBoost?", options:["Reduces training set size before training begins","For each tree, randomly samples a fraction of training examples — introducing randomness similar to Random Forest bagging, reducing overfitting","It determines how many features to sample per split","Controls gradient descent steps per tree"], correctAnswerIndex:1, explanation:"subsample=0.8 means each tree trains on random 80% of examples. This adds diversity between trees — additional regularisation on top of XGBoost's sequential correction." },
  ],
  ...
  "kmeans": [
    ...
    { id:2, question:"What does inertia measure, and what is wrong with minimising it alone?", options:["Inertia measures cluster separation — higher is better","Sum of squared distances from each point to its nearest centroid. Problem: inertia always decreases as k increases — k=N gives inertia=0. Use the elbow method or Silhouette score to choose k meaningfully","Inertia measures convergence speed","Inertia should be maximised"], correctAnswerIndex:1, explanation:"k=N (each point is its own cluster) perfectly minimises inertia but is useless. The elbow method finds k where improvement plateaus meaningfully." },
    { id:3, question:"K-Means fails to detect two interlocking crescent shapes. Why?", options:["K-Means cannot process more than 1000 examples","K-Means defines clusters by proximity to a single centroid — it can only create convex Voronoi regions. Crescent shapes cannot be represented as nearest to a central point. DBSCAN handles non-convex shapes","K-Means requires binary features","K-Means assigns each point to exactly one cluster"], correctAnswerIndex:1, explanation:"The Voronoi diagram created by K-Means centroids always produces convex regions. Non-convex cluster shapes (rings, crescents, spirals) violate this geometric constraint." },
    { id:4, question:"How does the Silhouette Score evaluate clustering quality?", options:["It counts correctly assigned cluster labels","For each point: a=average distance to same-cluster points, b=average distance to nearest other-cluster points, Silhouette=(b-a)/max(a,b). Values near +1 mean well-clustered; near -1 mean possibly in wrong cluster","It measures how fast K-Means converged","Ratio of inertia to number of clusters"], correctAnswerIndex:1, explanation:"Silhouette score captures both cohesion (how tight clusters are) and separation (how far clusters are from each other). Average above 0.5 generally indicates meaningful cluster structure." },
  ],

  "hierarchical": [
    { id:1, question:"What is the key advantage of hierarchical clustering over K-Means?", options:["It is always faster","You do not need to specify k in advance — the dendrogram shows the full hierarchy and you choose the cut level after seeing the structure","It works with non-Euclidean distances only","It guarantees globally optimal clusters"], correctAnswerIndex:1, explanation:"The dendrogram reveals hierarchical structure. You might find k=3 and k=7 both make sense depending on the business question. Cutting at different heights gives different cluster counts from a single run." },
    { id:2, question:"What does Ward linkage minimise when merging clusters?", options:["Maximum distance between any two points","Increase in total within-cluster variance from the merge — Ward tends to create compact, similarly-sized clusters","Number of points in the smaller cluster","Computational cost of the merge"], correctAnswerIndex:1, explanation:"Ward linkage is analogous to K-Means' objective — minimise within-cluster sum of squares. It tends to produce compact, interpretable clusters." },
    { id:3, question:"Hierarchical clustering has O(N^2) space and O(N^3) time. What does this mean practically?", options:["It is the fastest algorithm for large datasets","For N=100,000 examples, the distance matrix requires billions of entries — hierarchical clustering is impractical for large datasets without approximations","Time complexity only matters for quality, not result","It scales linearly with the square root of samples"], correctAnswerIndex:1, explanation:"In practice, hierarchical clustering works well for N < ~10,000. For larger datasets, use K-Means with multiple restarts, DBSCAN, or approximate hierarchical methods." },
  ...
  "dbscan": [
    { id:1, question:"What are the three types of points in DBSCAN?", options:["Cluster points, border points, and noise — but all belong to clusters","Core points (min_samples neighbours within epsilon), border points (within epsilon of a core point but fewer neighbours), and noise points (isolated — belong to no cluster)","Input, output, and unclassified points","Dense, sparse, and outlier points"], correctAnswerIndex:1, explanation:"Core points form the dense backbone. Border points are on the edge. Noise points are anomalies not assigned to any cluster — DBSCAN performs outlier detection automatically." },
    { id:2, question:"How do you choose epsilon and min_samples for DBSCAN?", options:["Always use epsilon=0.5 and min_samples=5","Plot K-nearest neighbour distances sorted ascending (K-distance graph) — look for the elbow where distance increases sharply. This elbow value is a good epsilon estimate. min_samples is typically 2 times dimension or 5","Epsilon should equal the mean feature value","These are set automatically"], correctAnswerIndex:1, explanation:"The K-distance graph is the standard diagnostic for epsilon selection. The elbow separates dense cluster points from noise points." },
    ...
    { id:4, question:"DBSCAN cannot handle clusters of vastly different densities. Why?", options:["DBSCAN requires all clusters to have the same number of points","A single epsilon is used globally. Dense clusters need small epsilon; sparse clusters need large epsilon. No single epsilon can correctly identify both. HDBSCAN adapts epsilon locally and solves this","DBSCAN assigns all points to the most dense cluster","DBSCAN has a maximum density limit"], correctAnswerIndex:1, explanation:"With one dense and one sparse cluster, epsilon tuned for the dense one makes the sparse one appear as all noise. HDBSCAN solves this with hierarchical density estimation." },
  ],

  "pca": [
    { id:1, question:"What do the principal components in PCA represent?", options:["Original features sorted by importance","New orthogonal axes (linear combinations of original features) ordered by variance explained — PC1 explains the most, PC2 explains the next most while perpendicular to PC1","Clusters in the data","Features with lowest correlation to target"], correctAnswerIndex:1, explanation:"PC1 is the direction in feature space with most variance. By projecting onto the first few PCs, you retain the most important variance while discarding dimensions that mostly contain noise." },
    ...
    { id:3, question:"Why must you standardise features before applying PCA?", options:["PCA raises an error on unstandardised features","PCA finds directions of maximum variance. A feature in thousands automatically dominates the first PC due to scale alone, not actual importance. Standardisation puts all features on equal footing","Standardisation improves PCA speed by 10x","PCA requires binary features"], correctAnswerIndex:1, explanation:"Without StandardScaler, PCA's maximum variance direction is essentially just the direction of the feature with the largest scale — an artifact of units, not true data structure." },
    { id:4, question:"How do you decide how many principal components to keep?", options:["Always keep exactly half","Plot cumulative explained variance and choose components explaining 90-95% of total variance — the knee of the scree plot. For downstream models, cross-validate with different numbers and choose based on validation performance","Keep all components — removing any always hurts","Always use exactly 2 for visualisation"], correctAnswerIndex:1, explanation:"95% retained variance is a common heuristic. For visualisation, 2-3 components. For downstream models, cross-validate with different numbers of components." },
  ],
  ...
  "tsne": [
    ...
    { id:2, question:"What does the perplexity parameter control in t-SNE?", options:["The number of output dimensions","The effective number of nearest neighbours for each point's local structure — low perplexity emphasises local structure; high perplexity considers broader neighbourhood. Try multiple perplexity values before drawing conclusions","The learning rate of t-SNE optimisation","The number of iterations"], correctAnswerIndex:1, explanation:"Low perplexity creates tight isolated clusters. High perplexity shows broader global structure but small clusters may disappear. Always try multiple perplexity values." },
    { id:3, question:"A t-SNE plot shows 5 clearly separated clusters. Can you conclude the data has exactly 5 natural clusters?", options:["Yes — t-SNE faithfully represents cluster structure","Not necessarily — t-SNE is stochastic and distorts global distances. Different random seeds produce different layouts. Treat as hypothesis generator, validate with quantitative clustering metrics","Yes, if perplexity was correctly chosen","Only if each cluster has the same number of points"], correctAnswerIndex:1, explanation:"Clusters that appear far apart in 2D may not be far apart in the original space. t-SNE suggests structure but does not confirm it definitively." },
    { id:4, question:"Why does t-SNE use a t-distribution in the low-dimensional space rather than a Gaussian?", options:["The t-distribution is easier to compute","The t-distribution has heavier tails — it allows moderate-distance neighbours in high dimensions to be placed far apart in 2D without penalty (the crowding problem). Gaussian in 2D would force too many points to cluster centrally","The t-distribution handles missing values better","A historical choice with no technical reason"], correctAnswerIndex:1, explanation:"The crowding problem: in 2D there is not enough space to faithfully represent all moderate-distance neighbours from high dimensions. The t-distribution's heavy tails allow t-SNE to place these points far apart, revealing cluster structure more clearly." },
  ],

  "association-rules": [
    { id:1, question:"What do Support, Confidence, and Lift measure in association rule mining?", options:["Three names for the same measure","Support: how frequently items appear together. Confidence: given item A, how often does B also appear. Lift: whether A and B appear together more than if independent — Lift > 1 means genuine association beyond chance","Support = transactions; Confidence = profit; Lift = cost","These metrics only apply to Apriori"], correctAnswerIndex:1, explanation:"All three are needed together. High confidence can be misleading — bread and milk appear frequently independently. Lift corrects for this: Lift=1 means independence, no genuine association." },
    { id:2, question:"Support(bread)=0.6, Support(milk)=0.7, Support(bread AND milk)=0.42. What is the Lift of bread to milk?", options:["0.42 / 0.6 = 0.7","Lift = 0.42 / (0.6 x 0.7) = 1.0 — no association beyond independence","0.6 x 0.7 = 0.42","0.42 minus 0.6 = -0.18"], correctAnswerIndex:1, explanation:"Lift = Support(A AND B) / (Support(A) x Support(B)) = 0.42/0.42 = 1.0. These items appear together exactly as often as independent items would — no meaningful association." },
    { id:3, question:"How do you filter hundreds of rules for genuinely useful ones?", options:["Keep all rules — more is better","Set minimum thresholds for Support (eliminates rare combinations), Confidence (eliminates unreliable rules), and Lift (eliminates spurious associations from item popularity). Then rank remaining rules by Lift","Remove all rules where either item has Support > 0.5","Use only the first 10 alphabetically"], correctAnswerIndex:1, explanation:"Without minimum support: unreliable rules from rare combinations. Without minimum confidence: rules with many exceptions. Without minimum lift: rules driven purely by item popularity." },
  ...
  "apriori": [
    { id:1, question:"What is the Apriori principle and why does it make the algorithm efficient?", options:["All rules with confidence above 0.5 are automatically frequent","If an itemset is infrequent (below minimum support), all its supersets must also be infrequent — this anti-monotone property allows pruning entire branches of the search space without evaluating them","Frequent itemsets always contain fewer than 5 items","Items must appear alphabetically to be considered frequent"], correctAnswerIndex:1, explanation:"Without this principle you would evaluate 2^N itemsets (exponential). With it: if milk-bread is infrequent, milk-bread-eggs can be immediately discarded. This converts an exponential search into a tractable computation." },
    { id:2, question:"Apriori generates 3-itemsets from 2-itemsets. What is the joining step?", options:["Randomly combine pairs of 2-itemsets","Combine two k-1-itemsets that share their first k-2 items — only matching-prefix itemsets are joined, generating each candidate exactly once","All 2-itemsets are combined with all others","Add the least frequent item to each 2-itemset"], correctAnswerIndex:1, explanation:"{A,B} joined with {A,C} produces {A,B,C} only if they share the same first k-2 items. This canonical join prevents generating the same candidate twice." },
    ...
    { id:4, question:"Minimum support 0.01% gives 50,000 rules; 5% gives 3 rules. How do you find a middle ground?", options:["Always use minimum support that gives exactly 100 rules","Try values between 0.01% and 5% iteratively, additionally filtering by minimum confidence and minimum Lift — all three thresholds together control rule set size and quality simultaneously","Use only the rule with highest Lift","Set minimum support equal to average item frequency"], correctAnswerIndex:1, explanation:"Business context matters: 5% support in a supermarket with 10M transactions means 500,000 transactions — a very strong signal. The combination of all three thresholds gives you control over quality and quantity." },
  ],
...
Object.assign(quizData, {
  ...
  "generative-vs-discriminative": [
    ...
    },
    {
      ...
      options: ["30 / 100 = 0.30", "36 / 100 = 0.36", "30 / 40 = 0.75", "30 / 36 ≈ 0.833"],
  ...
  "vae": [
    {
      ...
      options: ["μ", "log σ²", "ε sampled from N(0, I)", "The decoder weights"],
      correctAnswerIndex: 2,
    ...
    },
    {
      ...
      options: ["[0.9, -1.3]", "[0.7, -1.6]", "[0.2, -0.6]", "[0.5, 2.0]"],
      correctAnswerIndex: 1,
    ...
    },
    {
      ...
      options: ["Mode collapse", "Posterior collapse", "A missing random seed", "Perfect latent disentanglement"],
      correctAnswerIndex: 1,
  ...
  "gans": [
    ...
    },
    {
      ...
      options: ["-ln(0.90) - ln(0.80) ≈ 0.328", "-ln(0.20) ≈ 1.609", "0.90 + 0.20 = 1.10", "1 - 0.90 = 0.10"],
      correctAnswerIndex: 0,
    ...
    },
    {
      ...
      options: ["Posterior collapse", "Mode collapse", "Timestep mismatch", "Excessive retrieval"],
      correctAnswerIndex: 1,
  ...
  "diffusion-models": [
    ...
    },
    {
      ...
      options: ["The training dataset", "The optimizer", "The scheduler", "The text prompt alone"],
      correctAnswerIndex: 2,
  ...
  "stable-latent-diffusion": [
    {
      ...
      options: ["8 times fewer", "16 times fewer", "48 times fewer", "64 times fewer"],
      correctAnswerIndex: 2,
    ...
    },
    {
      ...
      options: ["The VAE encoder", "Only the VAE decoder", "The optimizer", "The training labels"],
      correctAnswerIndex: 0,
  ...
  "controlling-diffusion-models": [
    ...
    },
    {
      ...
      options: ["A higher random seed", "A pose-based ControlNet-style condition", "Textual Inversion", "More unrelated negative prompts"],
      correctAnswerIndex: 1,
  ...
  "finetuning-image-models": [
    ...
    },
    {
      ...
      options: ["Full-model retraining", "DreamBooth", "Textual Inversion", "ControlNet pose conditioning"],
      correctAnswerIndex: 2,
  ...
  "audio-music-video-generation": [
    {
      ...
      options: ["8,000", "16,000", "32,000", "160,000"],
      correctAnswerIndex: 2,
    ...
    },
    {
      ...
      options: ["Symbolic musical events", "A final waveform only", "Independent image pixels", "A class-probability table"],
      correctAnswerIndex: 0,
  ...
  "choosing-generative-model": [
    ...
    },
    {
      ...
      options: ["A VAE used only for anomaly scoring", "A diffusion model", "A fixed email template", "A discriminative regressor"],
      correctAnswerIndex: 1,
  ...
  "building-genai-apps": [
    ...
    },
    {
      ...
      options: ["Invalid input", "Invalid structure", "Unsupported hardware", "Model-selection failure"],
      correctAnswerIndex: 1,
    ...
    },
    {
      ...
      options: ["Retrieval", "An autonomous tool call", "A random seed", "A new output schema"],
      correctAnswerIndex: 0,
  ...
  "how-generative-models-learn": [
    ...
    },
    {
      ...
      options: ["Forest", "Coast", "City", "No category"],
      correctAnswerIndex: 1,
    ...
    },
    {
      ...
      options: ["Autoregressive generation", "Adversarial generation", "Latent-variable reconstruction", "Diffusion denoising"],
      correctAnswerIndex: 0,
